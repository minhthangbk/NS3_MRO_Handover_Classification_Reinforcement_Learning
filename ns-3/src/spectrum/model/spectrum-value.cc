...
//	if(m_spectrumModel != x.m_spectrumModel) //THANG
//	{
//		NS_LOG_UNCOND("m_spectrumModel Id: " << m_spectrumModel->GetUid() << " x.m_spectrumModel Id: " << x.m_spectrumModel->GetUid());
//		NS_ASSERT(false);
//	}
	NS_ASSERT (m_spectrumModel == x.m_spectrumModel);
...
Sum (const SpectrumValue& x)
{
	...
	while (it1 != x.ConstValuesEnd ())
	{
		//	  NS_LOG_UNCOND("spectrum-phy SUM s " << s << " with "<< (*it1)); //THANG add for debug
		s += (*it1);
		++it1;
	}
	return s;
