...
class IsLandProfile : public Object {
public:
	IsLandProfile();
	virtual ~IsLandProfile();
	static TypeId GetTypeId();
	bool CheckIslandCondition(double curX, double curY);
	virtual bool DoCheckIslandCondition(double curX, double curY) = 0;
	double GetGain() const;
	virtual double DoGetGain() const = 0;
	//	//small circle coverage
	//	double centerX;
	//	double centerY;
	//	double radius;
	//	double gain;
...
class CircleIsland : public IsLandProfile{
public:
	CircleIsland();
	CircleIsland(double centerX, double centerY, double radius, double gain);
	static TypeId GetTypeId (void);
	virtual bool DoCheckIslandCondition(double curX, double curY);
	virtual double DoGetGain() const;
...
class RectangularIsland : public IsLandProfile{
public:
	RectangularIsland();
	RectangularIsland(double minX, double maxX, double minY, double maxY, double angle, double gain);
	static TypeId GetTypeId (void);
	virtual bool DoCheckIslandCondition(double curX, double curY);
	virtual double DoGetGain() const;
...
public:
	 ...
	 * Add by THANG
	 */
	void SetIslands(std::map< uint64_t, std::list< Ptr<IsLandProfile> > > islandList);
...
private:
	...
	/*
	 * THANG add this value to enable spectrum value
	 * creating island for specific cell
	 */
	bool m_enIsland;
	std::map< uint64_t, std::list< Ptr<IsLandProfile> > > m_islandList;
	std::map< uint16_t, Ptr<NormalRandomVariable> > m_randomShadowing;//Different random schemes for different eNBs
	uint64_t m_rvStream;
	double m_shadowingVariance;
        double m_varianceBound;
};
