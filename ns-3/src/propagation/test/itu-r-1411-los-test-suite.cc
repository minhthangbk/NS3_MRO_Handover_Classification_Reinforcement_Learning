...
ItuR1411LosPropagationLossModelTestCase::DoRun (void)
{
  ...
  std::ofstream outFile;
    outFile.open ("ThangItuR1411.txt", std::ios_base::out | std::ios_base::trunc);
    if (!outFile.is_open ()){
        NS_LOG_ERROR ("Can't open file " << "ThangItuR1411.txt");
  		return;
    }
  ...
  : TestSuite ("itu-r-1411-los", SYSTEM)
{
  ...
  AddTestCase (new ItuR1411LosPropagationLossModelTestCase (2e9, 100, 30, 1,  80.5838, "freq=2000MHz, dist=100m"), TestCase::QUICK);
  //AddTestCase (new ItuR1411LosPropagationLossModelTestCase (2.1140e9, 200, 30, 1,  87.4855, "freq=2114MHz, dist=100m"), TestCase::QUICK);
