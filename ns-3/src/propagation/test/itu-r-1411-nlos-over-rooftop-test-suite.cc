...
ItuR1411NlosOverRooftopPropagationLossModelTestCase::DoRun (void)
{
  ...
  std::ofstream outFile;
      outFile.open ("ThangItuR1411OverRooftop.txt", std::ios_base::out | std::ios_base::trunc);
      if (!outFile.is_open ()){
          NS_LOG_ERROR ("Can't open file " << "ThangItuR1411OverRooftop.txt");
    		return;
      }
  ...
  : TestSuite ("itu-r-1411-nlos-over-rooftop", SYSTEM)
{
  ...
  /**************THANG**************/
  AddTestCase (new ItuR1411NlosOverRooftopPropagationLossModelTestCase (2e9, 100, 30, 1, UrbanEnvironment, LargeCity, 111.158, "f=2114Mhz, dist=900, urban large city"), TestCase::QUICK);
  /*********************************/
}
