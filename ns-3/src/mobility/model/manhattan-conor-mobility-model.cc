/*
 * manhattan-conor-mobility-model.cc
 *
...
#include "manhattan-conor-mobility-model.h"
...
NS_OBJECT_ENSURE_REGISTERED (ManhattanCornerMobilityModel);

TypeId
ManhattanCornerMobilityModel::GetTypeId(void)
{
	static TypeId tid = TypeId ("ns3::ManhattanCornerMobilityModel")
	    .SetParent<MobilityModel> ()
	    .SetGroupName ("Mobility")
	    .AddConstructor<ManhattanCornerMobilityModel> ()
	    .AddAttribute ("MinSpeed",
	                   "Minimum speed value, [m/s]",
	                   DoubleValue (3),
	                   MakeDoubleAccessor (&ManhattanCornerMobilityModel::m_minSpeed),
	                   MakeDoubleChecker<double> ())
	    .AddAttribute ("MaxSpeed",
	                   "Maximum speed value, [m/s]",
	                   DoubleValue (5),
	                   MakeDoubleAccessor (&ManhattanCornerMobilityModel::m_maxSpeed),
	                   MakeDoubleChecker<double> ())
		.AddAttribute ("MaxPause",
				"Maximum pause time [s].",
				DoubleValue (0),
				MakeDoubleAccessor (&ManhattanCornerMobilityModel::m_maxPause),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MinPause",
				"Minimum pause time [s].",
				DoubleValue (0),
				MakeDoubleAccessor (&ManhattanCornerMobilityModel::m_minPause),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MaxX",
				"Maximum of X",
				DoubleValue (100),
				MakeDoubleAccessor (&ManhattanCornerMobilityModel::m_xMax),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MinX",
				"Minimum of X",
				DoubleValue (0),
				MakeDoubleAccessor (&ManhattanCornerMobilityModel::m_xMin),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MaxY",
				"Maximum of Y",
				DoubleValue (100),
				MakeDoubleAccessor (&ManhattanCornerMobilityModel::m_yMax),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MinY",
				"Minimum of Y",
				DoubleValue (0),
				MakeDoubleAccessor (&ManhattanCornerMobilityModel::m_yMin),
				MakeDoubleChecker<double> ())
	    ;
...
ManhattanCornerMobilityModel::ManhattanCornerMobilityModel()
{
...
void
ManhattanCornerMobilityModel::DoInitialize(void)
{
	...
	double init_xh = (m_xMax-m_xMin)*2;
	double init_xr = init_xh/(init_xh+(m_yMax-m_yMin)*2);
	double xInit;
	double yInit;
	...
	m_event = Simulator::ScheduleNow(&ManhattanCornerMobilityModel::BeginWalk, this);
	m_alreadyStarted = true;
	NotifyCourseChange();
	MobilityModel::DoInitialize();
//	NS_LOG_FUNCTION("ManhattanCornerMobilityModel::DoInitialize "
...
//void
//ManhattanCornerMobilityModel::Start()
//{
//
//}

void
ManhattanCornerMobilityModel::BeginWalk()
{
	m_helper.Update();
	m_helper.Pause();
...
//				<< " pos.x: " << curPos.x << " pos.y: " << curPos.y
//				<< " dst.x: " << dst.x << " dst.y: " << dst.y) ;
		m_event = Simulator::Schedule (Seconds(travelTime),&ManhattanCornerMobilityModel::BeginWalk, this);
	}
	else
	{//not turn
...
//					<< " pos.x: " << curPos.x << " pos.y: " << curPos.y
//					<< " dst.x: " << dst.x << " dst.y: " << dst.y);
			m_event = Simulator::Schedule (Seconds(pauseDuration), &ManhattanCornerMobilityModel::BeginWalk, this);
		}
	}
	NotifyCourseChange();
}
...
bool
ManhattanCornerMobilityModel::OutOfBounds(Vector pos)
{
	return ((pos.x < m_xMin) ||
			(pos.x > m_xMax) ||
			(pos.y < m_yMin) ||
...
bool
ManhattanCornerMobilityModel::MustTurn(Vector pos, uint8_t dir)
{
...
Vector
ManhattanCornerMobilityModel::AlignPosition(Vector pos)
...
Vector
ManhattanCornerMobilityModel::GetDestination(Vector pos, double dist, uint8_t dir)
{
	Vector dst;
	dst.z = 0;
	switch(dir)
...
Vector
ManhattanCornerMobilityModel::DoGetPosition() const
{
	m_helper.Update ();
...
void
ManhattanCornerMobilityModel::DoSetPosition (const Vector &position)
{

}

Vector
ManhattanCornerMobilityModel::DoGetVelocity() const
{
	return m_helper.GetVelocity();
}

int64_t
ManhattanCornerMobilityModel::DoAssignStreams (int64_t stream)
{
