...
public:
   ...
   * THANG
   */
  std::vector<Vector> GetVectorPositionList();
private:
...
public:
  ...
  /*
   * Dev by THANG
   * Set min, max of radius
   */
  void SetMinRho(double minR);
  void SetMaxRho(double maxR);

  double GetCurrentTheta() const;//THANG add
...
//  double m_curR;//THANG add current Radius
  mutable double m_curThe;//THANG add current Theta//mark as const to enable modification
};

///*
// * Developed by THANG
// * Randomly distribute UE in a doughnut
// */
//
//class RandomDoughnutPositionAllocator : public PositionAllocator
//{
//public:
//  /**
//   * Register this type with the TypeId system.
...
//   */
//  static TypeId GetTypeId (void);
//  RandomDoughnutPositionAllocator ();
//  virtual ~RandomDoughnutPositionAllocator ();
//
////  /**
////   * \brief Set the random variable that generates position radius
////   * \param theta random variable that represents the radius of a position in a random disc.
