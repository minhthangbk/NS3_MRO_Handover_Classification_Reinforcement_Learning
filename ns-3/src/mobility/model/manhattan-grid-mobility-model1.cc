/*
 * manhattan-grid-mobility-model2.cc
 *
...
#include "manhattan-grid-mobility-model1.h"
...
NS_OBJECT_ENSURE_REGISTERED (ManhattanGridMobilityModel1);

TypeId
ManhattanGridMobilityModel1::GetTypeId(void)
{
	static TypeId tid = TypeId ("ns3::ManhattanGridMobilityModel1")
	    .SetParent<MobilityModel> ()
	    .SetGroupName ("Mobility")
	    .AddConstructor<ManhattanGridMobilityModel1> ()
	    .AddAttribute ("MinSpeed",
	                   "Minimum speed value, [m/s]",
	                   DoubleValue (3),
	                   MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_minSpeed),
	                   MakeDoubleChecker<double> ())
	    .AddAttribute ("MaxSpeed",
	                   "Maximum speed value, [m/s]",
	                   DoubleValue (5),
	                   MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_maxSpeed),
	                   MakeDoubleChecker<double> ())
	    .AddAttribute ("XBlocks",
	                   "Number of blocks on x-axis",
					   UintegerValue (5),
					   MakeUintegerAccessor (&ManhattanGridMobilityModel1::m_xBlocks),
					   MakeUintegerChecker<uint8_t> ())
	   .AddAttribute ("YBlocks",
					  "Number of blocks on y-axis",
					  UintegerValue (5),
					  MakeUintegerAccessor (&ManhattanGridMobilityModel1::m_yBlocks),
					  MakeUintegerChecker<uint8_t> ())
	   .AddAttribute ("TurnProbability",
				  	  "Turn Probability",
					  DoubleValue (0.5),
					  MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_turnProb),
					  MakeDoubleChecker<double> ())
	   .AddAttribute ("UpdateDist",
				"minimum Y, [m]",
				DoubleValue (2),
				MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_updateDist),
				MakeDoubleChecker<double> ())
		.AddAttribute ("SpeedChangeProbability",
		     	"Speed Change Probability",
				DoubleValue (0.5),
				MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_speedChangePro),
				MakeDoubleChecker<double> ())
		.AddAttribute ("PauseProbability",
				"Pause Probability",
				DoubleValue (0.5),
				MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_pausePro),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MaxPause",
				"Maximum pause time [s].",
				DoubleValue (0),
				MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_maxPause),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MinPause",
				"Minimum pause time [s].",
				DoubleValue (0),
				MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_minPause),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MaxX",
				"Maximum of X",
				DoubleValue (100),
				MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_xMax),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MinX",
				"Minimum of X",
				DoubleValue (0),
				MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_xMin),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MaxY",
				"Maximum of Y",
				DoubleValue (100),
				MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_yMax),
				MakeDoubleChecker<double> ())
		.AddAttribute ("MinY",
				"Minimum of Y",
				DoubleValue (0),
				MakeDoubleAccessor (&ManhattanGridMobilityModel1::m_yMin),
				MakeDoubleChecker<double> ())
	    ;
...
ManhattanGridMobilityModel1::ManhattanGridMobilityModel1()
{
...
void
ManhattanGridMobilityModel1::DoInitialize(void)
{
	NS_ASSERT(m_xMax > m_xMin);
	NS_ASSERT(m_yMax > m_yMin);
	NS_ASSERT((m_yBlocks >= 2) && (m_xBlocks >= 2));
	...
	{//First move along x
		...
		do
		{
			...
			//Choose inner positions
		}
		while((yInit==m_yMin)||(yInit==m_yMax));
	...
	{//First move along y
		...
		}while((xInit==m_xMin) || xInit==m_xMax); //Choose inner positions
	...
	m_event = Simulator::ScheduleNow(&ManhattanGridMobilityModel1::BeginWalk, this);
	m_alreadyStarted = true;
	NotifyCourseChange();
	MobilityModel::DoInitialize();
//	NS_LOG_FUNCTION("ManhattanGridMobilityModel1::DoInitialize "
...
//void
//ManhattanGridMobilityModel1::Start()
//{
//
//}

void
ManhattanGridMobilityModel1::BeginWalk()
{
	m_helper.Update();
	m_helper.Pause();
	...
	bool turnAtCrossroads = ((m_gridDist <= m_dist)//going to reach a cross-road
...
//		}
		...
		{//turnAtCrossroads or OutOfBound
			mdist = m_gridDist;
			if(m_dist > mdist)
			{
				m_dist -= mdist;
		...
		dst = AlignPosition(GetDestination(curPos,mdist,m_curDir));//aligning destination again

		if(m_curDir < 2) //currently moving along y (Up or down)
		{
//			if(dst.x > m_xMin)
//			{
//				if(dst.x < m_xMax)
...
//			}
			...
			if((dst.y > m_yMin)&&(dst.y < m_yMax))
			{
				//				if((dst.x > m_xMin)&&(dst.x < m_xMax))
				//				{
				NS_ASSERT((dst.x > m_xMin)&&(dst.x < m_xMax));
				m_curDir = (uint8_t)(m_rndGen->GetValue() * 2.) + 2; //turn at cross road (move right or left)
				//				}
			}
			else if(m_curDir==0)//if currently moving up, then down
			{
				m_curDir=1;
			}
			else //if currently moving down, then up
			{
				m_curDir=0;
			}
		}//currently moving along x (right or left)
		else if((dst.x > m_xMin)&&(dst.x < m_xMax))
		{
			NS_ASSERT((dst.y > m_yMin)&&(dst.y < m_yMax));
			m_curDir = (uint8_t)(m_rndGen->GetValue() * 2.); //turn at cross road (move up or down)
		}
		else if(m_curDir==2)
		{
			m_curDir=3;
...
//		else if (dst.y > m_yMin)//currently moving along x (right or left)
//		{
//			if(dst.y < m_yMax)
//			{// in range [yMin, yMax]
...
//				<< " pos.x: " << curPos.x << " pos.y: " << curPos.y
//				<< " dst.x: " << dst.x << " dst.y: " << dst.y) ;
		m_event = Simulator::Schedule (Seconds(travelTime),&ManhattanGridMobilityModel1::BeginWalk, this);
	}
	else
	{//not turn
...
//					<< " pos.x: " << curPos.x << " pos.y: " << curPos.y
//					<< " dst.x: " << dst.x << " dst.y: " << dst.y);
			m_event = Simulator::Schedule (Seconds(pauseDuration), &ManhattanGridMobilityModel1::BeginWalk, this);
		}
	}
	NotifyCourseChange();
}
...
bool
ManhattanGridMobilityModel1::OutOfBounds(Vector pos)
{
	return ((pos.x < m_xMin) ||
			(pos.x > m_xMax) ||
			(pos.y < m_yMin) ||
...
bool
ManhattanGridMobilityModel1::MustTurn(Vector pos, uint8_t dir)
{
...
Vector
ManhattanGridMobilityModel1::AlignPosition(Vector pos)
...
Vector
ManhattanGridMobilityModel1::GetDestination(Vector pos, double dist, uint8_t dir)
{
	Vector dst;
	dst.z = 0;
	switch(dir)
...
Vector
ManhattanGridMobilityModel1::DoGetPosition() const
{
	m_helper.Update ();
...
void
ManhattanGridMobilityModel1::DoSetPosition (const Vector &position)
{

}

Vector
ManhattanGridMobilityModel1::DoGetVelocity() const
{
	return m_helper.GetVelocity();
}

int64_t
ManhattanGridMobilityModel1::DoAssignStreams (int64_t stream)
{
