...
/*
 * THANG add more function
 */
std::vector<Vector>
ListPositionAllocator::GetVectorPositionList(){
	return m_positions;
}
 ...
 */
void
RandomDiscPositionAllocator::SetMinRho(double minR){
	m_rho->SetAttribute("Min",DoubleValue(minR));
}
void
RandomDiscPositionAllocator::SetMaxRho(double maxR){
	m_rho->SetAttribute("Max",DoubleValue(maxR));
}
double
RandomDiscPositionAllocator::GetCurrentTheta() const{
	return m_curThe;
}
Vector
RandomDiscPositionAllocator::GetNext (void) const
{
  m_curThe = m_theta->GetValue ();
  double rho = m_rho->GetValue ();
  double x = m_x + std::cos (m_curThe) * rho;
  double y = m_y + std::sin (m_curThe) * rho;
...
// * Developed by THANG
// */
//NS_OBJECT_ENSURE_REGISTERED (RandomDoughnutPositionAllocator);
//
//TypeId
//RandomDoughnutPositionAllocator::GetTypeId (void)
//{
//  static TypeId tid = TypeId ("ns3::RandomDiscPositionAllocator")
//    .SetParent<PositionAllocator> ()
//    .SetGroupName ("Mobility")
//    .AddConstructor<RandomDoughnutPositionAllocator> ()
//    .AddAttribute ("Theta",
//                   "A random variable which represents the angle (gradients) of a position in a random disc.",
//                   StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=6.2830]"),
//                   MakePointerAccessor (&RandomDoughnutPositionAllocator::m_theta),
//                   MakePointerChecker<RandomVariableStream> ())
//    .AddAttribute ("Rho",
//                   "A random variable which represents the radius of a position in a random disc.",
//                   StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=200.0]"),
//                   MakePointerAccessor (&RandomDoughnutPositionAllocator::m_rho),
//                   MakePointerChecker<RandomVariableStream> ())
//    .AddAttribute ("X",
//                   "The x coordinate of the center of the random position disc.",
//                   DoubleValue (0.0),
//                   MakeDoubleAccessor (&RandomDoughnutPositionAllocator::m_x),
//                   MakeDoubleChecker<double> ())
//    .AddAttribute ("Y",
//                   "The y coordinate of the center of the random position disc.",
//                   DoubleValue (0.0),
//                   MakeDoubleAccessor (&RandomDoughnutPositionAllocator::m_y),
//                   MakeDoubleChecker<double> ())
////    .AddAttribute ("MaxR",
////    			   "Maximum radius of the disc",
////    			   DoubleValue (0.0),
////    			   MakeDoubleAccessor (&RandomDoughnutPositionAllocator::m_maxR),
////    			   MakeDoubleChecker<double> ())
////	.AddAttribute ("MinR",
////				   "Minimum radius of the disc",
////				   DoubleValue (0.0),
////				   MakeDoubleAccessor (&RandomDoughnutPositionAllocator::m_minR),
////				   MakeDoubleChecker<double> ())
//  ;
//  return tid;
//}
//
//RandomDoughnutPositionAllocator::RandomDoughnutPositionAllocator (){
////	m_theta = CreateObject<UniformRandomVariable> ();
////	m_theta->SetAttribute("Min",DoubleValue(0));
////	m_theta->SetAttribute("Max",DoubleValue(6.2830));
////
////	m_rho = CreateObject<UniformRandomVariable> ();
//}
//RandomDoughnutPositionAllocator::~RandomDoughnutPositionAllocator (){
//
//}
////
////void
////RandomDoughnutPositionAllocator::SetTheta (Ptr<RandomVariableStream> theta)
////{
////  m_theta = theta;
////}
////void
////RandomDoughnutPositionAllocator::SetRho (Ptr<RandomVariableStream> rho)
////{
////  m_rho = rho;
////}
////void
////RandomDoughnutPositionAllocator::SetX (double x)
////{
////  m_x = x;
////}
////void
////RandomDoughnutPositionAllocator::SetY (double y)
////{
////  m_y = y;
////}
//Vector
//RandomDoughnutPositionAllocator::GetNext (void) const
//{
//	NS_ASSERT(m_minR < m_maxR && m_minR > 0);
////	m_rho->SetAttribute("Min",DoubleValue(m_minR));
////	m_rho->SetAttribute("Max",DoubleValue(m_maxR));
...
//int64_t
//RandomDoughnutPositionAllocator::AssignStreams (int64_t stream)
//{
//  m_theta->SetStream (stream);
//  m_rho->SetStream (stream + 1);
//  return 2;
