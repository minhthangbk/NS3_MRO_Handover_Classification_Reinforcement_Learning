...
LteStatsCalculator::FindImsiFromEnbRlcPath (std::string path)
{
  ...
  if (match.GetN () != 0)
    ...
    {
      NS_FATAL_ERROR ("LteStatsCalculator::FindImsiFromEnbRlcPath Lookup " << ueMapPath << " got no matches");
    }
}
...
LteStatsCalculator::FindImsiFromEnbMac (std::string path, uint16_t rnti)
{
  ...
  NS_LOG_DEBUG("LteStatsCalculator::FindImsiFromEnbMac " << "RNTI " << rnti); //THANG
...
{
  //NS_LOG_FUNCTION (path << rnti);
  NS_LOG_DEBUG("LteStatsCalculator::FindImsiForEnb" << " RNTI " << rnti);
  uint64_t imsi = 0;
...
{
  //NS_LOG_FUNCTION (path << rnti);
	NS_LOG_DEBUG("LteStatsCalculator::FindImsiForUe" << " RNTI " << rnti);
  uint64_t imsi = 0;
