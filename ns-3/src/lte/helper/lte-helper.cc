    ...
    m_cellIdCounter (0)
{
  ...
  m_channelFactory.SetTypeId (MultiModelSpectrumChannel::GetTypeId ());//default values
}
/*Void SetSpectrumChannel(...)
 ...
 */
void
LteHelper::SetIslandDownlinkSpectrum(std::map< uint64_t, std::list< Ptr<IsLandProfile> > > islandList){
	m_downlinkChannel = m_channelFactory.Create<SpectrumChannel> ();
	Ptr<MultiModelSpectrumChannel> multi = DynamicCast< MultiModelSpectrumChannel >(m_downlinkChannel);
	NS_ASSERT(multi != NULL);
	multi->SetIslands(islandList);
}

void
LteHelper::SetInitialCIOValues(std::map<uint16_t,int8_t> cioVals){
	m_cioVals = cioVals;
}
...
{
  NS_LOG_FUNCTION (this);
  if(m_enIslandMode == false){//THANG modify
	  m_downlinkChannel = m_channelFactory.Create<SpectrumChannel> ();
  }
  m_uplinkChannel = m_channelFactory.Create<SpectrumChannel> ();
  NS_ASSERT(m_uplinkChannel != NULL);
  NS_ASSERT(m_downlinkChannel != NULL);
  //THANG test casting
//  Ptr<MultiModelSpectrumChannel> multi = DynamicCast< MultiModelSpectrumChannel >(m_downlinkChannel);
//  NS_ASSERT(multi != NULL);
...
TypeId LteHelper::GetTypeId (void)
{
  static TypeId
	...
	.AddAttribute("EnableIslandMode",
			"Enable Settings for island",
			BooleanValue(false),
			MakeBooleanAccessor(&LteHelper::m_enIslandMode),
			MakeBooleanChecker())
  ;
  return tid;
}
...
LteHelper::SetPathlossModelAttribute (std::string n, const AttributeValue &v)
{
  ...
  m_ulPathlossModelFactory.Set (n, v);//both uplink and downlink have same attribute values
}
...
NetDeviceContainer
LteHelper::InstallEnbDevice (NodeContainer c, std::vector<uint16_t> cellIdList)
{
	NS_LOG_FUNCTION (this);
	Initialize ();  // will run DoInitialize () if necessary
	NetDeviceContainer devices;
	uint16_t id = 0;
	NS_ASSERT_MSG(cellIdList.size()==c.GetN(),"Mismatch number of Enbs and number of CellId");
	for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
	{
		Ptr<Node> node = *i;
		Ptr<NetDevice> device = InstallSingleEnbDevice (node, cellIdList.at(id));
		devices.Add (device);
		id++;
	}
	return devices;
...
LteHelper::InstallSingleEnbDevice (Ptr<Node> n)
{
	...
	//THANG add for creating specific pathloss model
	mm->SetAttribute("IsUE",BooleanValue(false));
	mm->SetAttribute("NodeId",UintegerValue((uint64_t)cellId));
	...
	/* Developed by THANG
	 * Create DSON management layer
	 */
	Ptr<LteEnbDSON> dson = CreateObject<LteEnbDSON>();
	rrc->SetInitialCIOValues(m_cioVals);
	...
	/*
	 * Developed by THANG
	 * Set up RRC-DSON interface between DSON layer and RRC layer
	 */
	rrc->SetLteEnbDSONSapUser(dson->GetEnbDSONSapUser());
	dson->SetEnbDSONSapProvider(rrc->GetLteEnbDSONSapProvider());
	...
	/*
	 * Developed by THANG
	 * add DSON pointer into LteEnbNetDevice
	 */
	dev->SetAttribute("LteEnbDSON",PointerValue(dson));
	...
	NS_LOG_LOGIC ("LteHelper DL freq: " << dlFreq);
...
Ptr<NetDevice>
LteHelper::InstallSingleEnbDevice (Ptr<Node> n, uint16_t cellId)
{
...
LteHelper::InstallSingleUeDevice (Ptr<Node> n)
{
	...
	Ptr<MobilityModel> mm = n->GetObject<MobilityModel> ();
	//THANG add for discriminate UE and eNB Node
	mm->SetAttribute("IsUE",BooleanValue(true));
	mm->SetAttribute("NodeId",UintegerValue(imsi));
...
{ 
  NS_LOG_UNCOND("LteHelper ActivateDrb IMSI " << imsi <<" cellId: "<< cellId <<" RNTI: "<< rnti << m_active << " Bearer QoS: " << m_bearer.qci);
...
////THANG
//void
//LteHelper::EnableRLFTraces(void){
//	NS_LOG_FUNCTION_NOARGS ();
//	  Config::Connect ("/NodeList/*/DeviceList/*/LteUeRrc/RLFDetectionTrace",
//	                   MakeBoundCallback (&LteUeRrc::RLFDetectionCallback, m_phyStats));
//}
