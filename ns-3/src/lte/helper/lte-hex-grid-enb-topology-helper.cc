...
//THANG
void LteHexGridEnbTopologyHelper::ReportEnbPos(Vector enbPos)
{
  std::ofstream outFile;
  if ( m_EnbFirstWrite == true )
    {
      outFile.open ("EnbPos.txt");
      if (!outFile.is_open ())
        {
          NS_LOG_ERROR ("Can't open file " << "EnbPos.txt");
          return;
        }
      m_EnbFirstWrite = false;
      //outFile << "% time\tcellId\EnbPos";
      //outFile << std::endl;
    }
  else
    {
      outFile.open ("EnbPos.txt",  std::ios_base::app);
      if (!outFile.is_open ())
        {
          NS_LOG_ERROR ("Can't open file " << "EnbPos.txt");
          return;
  ...
  //Append new strings
  outFile << enbPos.x << "," << enbPos.y << "," << enbPos.z << std::endl;
  outFile.close ();
}
