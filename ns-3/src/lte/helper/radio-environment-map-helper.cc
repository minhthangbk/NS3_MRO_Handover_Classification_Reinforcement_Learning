...
void
RadioEnvironmentMapHelper::SetIslands(std::map< uint64_t, std::list< Ptr<IsLandProfile> > > islandList){
...
RadioEnvironmentMapHelper::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::RadioEnvironmentMapHelper")
   ...
   .AddAttribute ("XRes", "The resolution (number of points) of the map along the x axis.",
	...
	.AddAttribute ("EnableRSRPPlot",
				   "Enable plotting RSRP, default value is false",
				   BooleanValue (false),
				   MakeBooleanAccessor (&RadioEnvironmentMapHelper::m_enPlotRSRP),
				   MakeBooleanChecker())
  ;
  return tid;
}
...
RadioEnvironmentMapHelper::DelayedInstall ()
{
	...
	NS_LOG_UNCOND("EARFCN: " << m_earfcn << " bandwidth: " << m_bandwidth);
	for (uint32_t i = 0; i < m_maxPointsPerIteration; ++i)
	{
		...
		if(m_enIsland){
			p.phy->SetRemIslands(m_islandList);
		}
...
RadioEnvironmentMapHelper::PrintAndReset ()
{
			...
			++it)
	{
		...
		m_outFile << pos.x << "\t"
				<< pos.y << "\t"
				<< pos.z << "\t"
				<< ((m_enPlotRSRP) ? (10*std::log10(it->phy->GetRSRP())) :
						(it->phy->GetSinr (m_noisePower))) << "\t"
						<< it->phy->GetCellId()
						<< std::endl;
		it->phy->Reset ();
