...
public:
  ...
  NetDeviceContainer InstallEnbDevice (NodeContainer c, std::vector<uint16_t> cellIdList);
 ...
 * Added by THANG
 */
  void SetIslandDownlinkSpectrum(std::map< uint64_t, std::list< Ptr<IsLandProfile> > > islandList);

  void SetInitialCIOValues(std::map<uint16_t,int8_t> cioVals);
  /** 
   * 
  ...
  */
  ...
  Ptr<NetDevice> InstallSingleEnbDevice (Ptr<Node> n, uint16_t cellId);
...
//  Ptr<NetDevice> InstallSingleEnbDevice (Ptr<Node> n, uint16_t cellId);
  ...
  std::map<uint16_t,int8_t> m_cioVals;
