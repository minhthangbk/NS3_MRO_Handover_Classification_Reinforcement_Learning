...
LteUeRrc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUeRrc")
    ...
    .AddTraceSource ("HandoverEndError",
                     ...
                     "ns3::LteUeRrc::ImsiCidRntiTracedCallback")
//    .AddTraceSource("RLFDetectionTrace",   //THANG
//                    "trace RLF events",
//                    MakeTraceSourceAccessor(&LteUeRrc::m_RLFDetectionTrace),
//                    "ns3::LteUeRrc::RLFDetectionCallback")
  ;
  return tid;
}
...
LteUeRrc::DoReportUeMeasurements (LteUeCphySapUser::UeMeasurementsParameters params)
{
  ...
  for (newMeasIt = params.m_ueMeasurementsList.begin ();
    ...
    {
      /****************************************THANG****************************************/
      //Connected Normally: No RLF recovery ongoing (T310 not running), no HO ongoing (T304 not running),
	  //no waiting RRC Re-establishment Establishment/Reject (T301 not running), no waiting RRC Connection Setup/Reject (T300 not running)
        ...
        (m_state == CONNECTED_NORMALLY)) //out of sync happens      
      {
    	  ...
    	  RSRQTracing(m_cellId, m_imsi, m_rnti, newMeasIt->m_rsrq); //Save serving cell RSRQ into log file
          if (newMeasIt->m_outofsyncFlag){//out of sync indication
              m_outofsyncCnt++;
              m_insyncCnt = 0;
              if(m_outofsyncCnt >= m_N310){//Trigger RLF recovery
                  m_outofsyncCnt = 0;
                  m_insyncCnt = 0;

                  m_rlfMeas.m_rsrq = newMeasIt->m_rsrq; //Save measurements for logging
                  m_rlfMeas.m_rlfhappen = true;

                  if(!m_T311Timeout.IsRunning()){//report 1 time after RLF
                      m_T311Timeout = Simulator::Schedule (m_T310,&LteUeRrc::ReportRLFDetection,this,m_cellId,m_imsi,m_rnti,m_rlfMeas.m_rsrq,m_rlfMeas.m_rlfhappen);
                  }
              }
          }else{//in sync
              m_outofsyncCnt = 0;
              ...
              if(m_insyncCnt >= m_N311){
                  m_outofsyncCnt = 0;
                  m_insyncCnt = 0;
                  if(m_T311Timeout.IsRunning()){
                      m_T311Timeout.Cancel();
                  }
                  //Notify in-sync???
              }
          }
      ...
      SaveUeMeasurements (newMeasIt->m_cellId, newMeasIt->m_rsrp,newMeasIt->m_rsrq, useLayer3Filtering, //for saving data
    		  (newMeasIt == params.m_ueMeasurementsList.begin())); //for printing
      /*************************************************************************************/
...
//THANG
void LteUeRrc::RLFRecoveryTimeout ()
{
//print out notification
NS_LOG_FUNCTION ("RLF Detection");
//ReportRLFDetection(m_cellId, m_imsi, m_rnti, m_rlfMeas.m_rsrq, m_rlfMeas.m_rlfhappen);
...
void LteUeRrc::RSRQTracing(uint16_t cellId, uint64_t imsi, uint16_t rnti, double rsrq)
{
...
if ( m_RSRQFirstWrite == true )
{
  ...
  outFile << "% time\tcellId\tIMSI\tRNTI\tRSRQ";
  outFile << std::endl;
}
else
{
...
void
LteUeRrc::SaveUeMeasurements (uint16_t cellId, double rsrp, double rsrq, bool useLayer3Filtering, bool newMeas)
{
...
{
NS_LOG_INFO ("Write All Measurements in " << "THANG_UeMeasurements.txt");

std::ofstream outFile;
if ( m_MeasurementFirstWrite == true )
{
  outFile.open ("THANG_UeMeasurements.txt");
  if (!outFile.is_open ())
	{
	  NS_LOG_ERROR ("Can't open file " << "THANG_UeMeasurements.txt");
	  return;
	}
  m_MeasurementFirstWrite = false;
  outFile << "% time\tcellId\tIMSI\tRNTI\t {cellID,RSRP,RSRQ}";
  //outFile << std::endl;
}
else
{
  outFile.open ("THANG_UeMeasurements.txt",  std::ios_base::app);
  if (!outFile.is_open ())
	{
	  NS_LOG_ERROR ("Can't open file " << "THANG_UeMeasurements.txt");
	  return;
