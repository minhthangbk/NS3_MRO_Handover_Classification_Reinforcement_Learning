...
public:
	...
	virtual void ConfigureMac (uint8_t ulBandwidth, uint8_t dlBandwidth, uint16_t cellId);
...
void
EnbMacMemberLteEnbCmacSapProvider::ConfigureMac (uint8_t ulBandwidth, uint8_t dlBandwidth, uint16_t cellId)
{
	m_mac->DoConfigureMac (ulBandwidth, dlBandwidth, cellId);
}
...
LteEnbMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteEnbMac")
    ...
    .AddAttribute ("NumberOfRaPreambles",
                   ...
                   UintegerValue (40), //THANG modify to make more handovers
	...
	.AddTraceSource("DlResAlloTrace", "Trace resource allocation information",
					MakeTraceSourceAccessor(&LteEnbMac::m_resAllTracedCallback),
					"ns3::LteEnbMac::ResAlloTracedCallback")
  ;
...
{
	NS_LOG_LOGIC ("LteEnbMac::DoSubframeIndication EnbMac - frame " << frameNo << " subframe " << subframeNo);
...
void
LteEnbMac::DoReceivePhyPdu (Ptr<Packet> p) //Receive Uplink Data
{
	NS_LOG_FUNCTION (this);
	...
	std::map <uint16_t, std::map<uint8_t, LteMacSapUser*> >::iterator rntiIt = m_rlcAttached.find (tag.GetRnti ());
	if(rntiIt != m_rlcAttached.end ()){//Accept the uplink packet only if the RNTI is existed, otherwise, neglect it
		std::map<uint8_t, LteMacSapUser*>::iterator lcidIt = rntiIt->second.find (tag.GetLcid ());
		//NS_ASSERT_MSG (lcidIt != rntiIt->second.end (), "could not find LCID" << lcid);
...
void
LteEnbMac::DoConfigureMac (uint8_t ulBandwidth, uint8_t dlBandwidth, uint16_t cellId)
{
...
{
	NS_LOG_LOGIC("LteEnbMac::DoAddUe cellId " << m_cellId << " RNTI " << rnti);
...
{
	NS_LOG_FUNCTION ("LteEnbMac::DoRemoveUe cellId " << m_cellId << " RNTI " << rnti);
...
LteEnbMac::DoGetRachConfig ()
{
	...
	rc.numberOfRaPreambles = m_numberOfRaPreambles; //Total #Preambles for contention basd RACH
...
LteEnbCmacSapProvider::AllocateNcRaPreambleReturnValue 
LteEnbMac::DoAllocateNcRaPreamble (uint16_t rnti) //Non-Contention based RACH (Handover)
{
	bool found = false;
	uint8_t preambleId;
...
LteEnbMac::DoSchedDlConfigInd (FfMacSchedSapUser::SchedDlConfigIndParameters ind)
{
	...
	for ( uint32_t i  = 0; i < ind.m_buildDataList.size (); i++ ){
		...
		if (ind.m_buildDataList.at (i).m_dci.m_tbsSize.size () == 1){
			...
			m_dlScheduling (m_frameNo, m_subframeNo,
					ind.m_buildDataList.at (i).m_dci.m_rnti,
			...
			//Thang's Trace source
			SONCommon::ResourceAllocationInfo resAllInfo;
			resAllInfo.frame = m_frameNo;
			resAllInfo.subframe = m_subframeNo;
			resAllInfo.rnti = ind.m_buildDataList.at (i).m_dci.m_rnti;
			resAllInfo.rbBitmap = ind.m_buildDataList.at (i).m_dci.m_rbBitmap;
			resAllInfo.tbs0Size = ind.m_buildDataList.at (i).m_dci.m_tbsSize.at (0);
			resAllInfo.rbRIV = ind.m_buildDataList.at (i).m_dci.m_rbRIV;
			m_resAllTracedCallback(m_cellId,resAllInfo);

		}else if (ind.m_buildDataList.at (i).m_dci.m_tbsSize.size () == 2){ // Two TBs used
			...
			resAllInfo.rnti = ind.m_buildDataList.at (i).m_dci.m_rnti;
			resAllInfo.rbBitmap = ind.m_buildDataList.at (i).m_dci.m_rbBitmap;
			resAllInfo.tbs1Size = ind.m_buildDataList.at (i).m_dci.m_tbsSize.at (1);
			resAllInfo.rbRIV = ind.m_buildDataList.at (i).m_dci.m_rbRIV;
			m_resAllTracedCallback(m_cellId,resAllInfo);
...
LteEnbMac::DoSchedUlConfigInd (FfMacSchedSapUser::SchedUlConfigIndParameters ind)
{
	...
	{
		NS_LOG_LOGIC("LteEnbMac::DoSchedUlConfigInd cellId " << m_cellId << " RNTI " << ind.m_dciList.at (i).m_rnti << " rbStart " << (uint32_t)ind.m_dciList.at (i).m_rbStart <<
				" rbLen " << (uint32_t)ind.m_dciList.at (i).m_rbLen <<
				" at m_frameNo " << m_frameNo << " m_subframeNo " << m_subframeNo);
