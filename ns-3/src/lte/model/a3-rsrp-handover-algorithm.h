...
protected:
  ...
  //THANG add uint16_t cellId for logging
  void DoReportUeMeas (uint16_t cellId, uint16_t imsi, uint16_t rnti, LteRrcSap::MeasResults measResults);
...
public:
  /*
     * dev by THANG
     * save concerned list before handover
     * cellid
     */
    void TraceCandidateCellForHandover(uint16_t cellId, uint16_t targetCellId, uint8_t rsrpTarget, LteRrcSap::MeasResults measResults);
    bool m_logFirstWrite;
    TracedCallback<uint16_t, uint16_t, uint16_t, LteRrcSap::MeasResults> m_recvMeasReportA3;
    typedef void (* ReceiveMeasReportA3) (const uint16_t cellId, const uint16_t targetCellId, const uint16_t imsi, const LteRrcSap::MeasResults);
