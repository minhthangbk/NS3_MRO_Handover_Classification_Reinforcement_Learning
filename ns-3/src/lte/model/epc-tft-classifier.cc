...
EpcTftClassifier::EpcTftClassifier () //epc-ue-nas also uses the class
{
...
EpcTftClassifier::Add (Ptr<EpcTft> tft, uint32_t id)
{
	...
	m_tftMap[id] = tft;
//	NS_LOG_UNCOND("EpcTftClassifier::Add #Bearer-TFT mappings m_tftMap.size: " << (uint32_t)m_tftMap.size () << " TeId: " << (uint32_t)id);
	// simple sanity check: there shouldn't be more than 16 bearers (hence TFTs) per UE
	NS_ASSERT (m_tftMap.size () <= 16);
}

void
EpcTftClassifier::ClearTftMap()
{
	m_tftMap.clear();
}

std::map <uint32_t, Ptr<EpcTft> >
EpcTftClassifier::GetTftMap(){
	return m_tftMap;
}
