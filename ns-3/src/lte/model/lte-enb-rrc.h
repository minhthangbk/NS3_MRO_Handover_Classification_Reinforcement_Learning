...
public: 
  ...
  /***************THANG***************/
  std::map <uint8_t, Ptr<LteDataRadioBearerInfo> > GetDataRadioBearerMapping();
  void RecvRrcConnectionReestablishmentRequest (LteRrcSap::RrcConnectionReestablishmentRequest msg, bool UeContextExisted);
  /***********************************/
  ...
  /********************THANG**********************/
  /*
   * Change ReportConfig request
   */
  void ModifyReportConfigRequest(LteRrcSap::RrcConnectionReconfiguration msg);
...
//  uint8_t GetNewRrcTransactionIdentifier ();
  ...
  /**************THANG***************/
  EventId m_connectionReestablishmentTimeout;
  /**********************************/
...
public:
  ...
  /*************************THANG*************************/
  void ConnectionReestablishmentTimeout(uint16_t rnti);
  void DoSendReleaseListDataRadioBearers (std::list<uint8_t> bearerId, uint64_t imsi, uint16_t rnti);
  void RemoveUeDueToRLF(uint16_t rnti);
  void RemoveUeDueToConnectionReestablishmentReject(uint16_t rnti);
  /*******************************************************/
   ...
   * THANG add Callback
   */
  typedef void (* ReceiveMroStatisticalData) (const uint16_t cellId, const SONCommon::MROProblemInfo prob);

  typedef void (* ReceiveA3MeasReport) (const uint16_t cellId, const uint64_t imsi, const uint16_t rnti, const LteRrcSap::MeasResults); //uint64_t, uint16_t, uint16_t, double

  typedef void (* ReceiveA4MeasReport) (const uint16_t sCellId, const uint16_t rnti, const LteRrcSap::MeasurementReport measReport);
private:
  ...
  /***************THANG****************/
  void DoRecvRlfIndicationMessage(EpcX2SapUser::RlfIndicationMessageParams params);
  /************************************/
  // CMAC SAP methods
...
/**************THANG***************/
  Time m_connectionReestablishmentTimeoutDuration;
/**********************************/
  ...
  /*********************THANG********************/
  /*
   * Statistic Callback is used for continuous monitoring
   */
  TracedCallback< uint16_t, SONCommon::MROProblemInfo > m_recvMroStatData;

  TracedCallback<uint16_t, uint64_t, uint16_t, LteRrcSap::MeasResults> m_recvA3MeasReportCallback; //imsi, serving cellId, rnti, & corresponding Mp (dB)

  TracedCallback<uint16_t, uint16_t, LteRrcSap::MeasurementReport> m_recvA4MeasReportCallback; //serving cell Id, target cell, Mn, Mp, RNTI
public:
  struct HandOverUeContext{
	  ...
	  bool m_whoDetected;
	  bool m_ppDetected;
	  std::list<LteRrcSap::MeasResultEutra> m_rlfMeasData;
//	  double updateTime;
  };
  void RemoveHandoverdUeContextForEHO(HandOverUeContext hoUeContext);
  std::list<HandOverUeContext> m_UeContextForEHO;
  Time m_UeCxtTimerForEHO; //Early HO
  void RemoveHandoverdUeContextForWHO(HandOverUeContext hoUeContext);
  std::list<HandOverUeContext> m_UeContextForWHO;
  Time m_UeCxtTimerForWHO; // Wrong Cell HO
  void SetUeCxtStoreTimer(Time time);
  Time GetUeCxtStoreTimer() const;

//  uint16_t m_NumTooEarlyHO;
//  uint16_t m_NumTooLateHO;
//  uint16_t m_NumWrongCellHO;
//  uint16_t m_NumPingpongHO;
//  uint16_t m_NumHOs;
  Time m_lastUpdateTime;//To avoid old context notification, it may happen in ping-pong handover
//  uint64_t m_aggSq; //Aggregated variance
//  uint64_t m_aggSum; //Aggregated variance
//  uint64_t m_aggNumHO;

  void SetHandoverControlParams(double a3Offset, uint16_t timeToTrigger);
  void SetANRControlParams(uint8_t rsrpThreshold);
  void SaveHystA3OffsetTTTToLog(double hyst, double a3ofsetVal, uint16_t tttVal);
  bool m_A3TTTFirstWrite;
  LteRrcSap::RrcConnectionReconfiguration BuildRrcConnectionReconfigurationForHandoverParameters(double hysteresis, uint16_t timeToTrigger);
  LteRrcSap::RrcConnectionReconfiguration BuildRrcConnectionReconfigurationWithoutDedicatedConfig();
  LteRrcSap::RrcConnectionReconfiguration BuildRrcConnectionReconfigurationForANR(uint8_t rsrpThreshold);
  uint8_t m_curMeasConfigIdHO;
  uint8_t m_curMeasConfigIdAnr;
  std::map<uint8_t,double> m_a3OffVals;
  std::map<uint8_t,uint16_t> m_tttVals;

  double m_curHysVal;//Save previous settings
  double m_curA3OffVal;
  uint16_t m_curTTTVal;

  void SetupCellToAddModList();
  void InitializeDSONParams();
  void SetInitialCIOValues(std::map<uint16_t,int8_t> cioVals);//Call from LteHelper;
  std::map<uint16_t,int8_t> m_cellsToAddModMap;

  /*
   * Database for each neighbor cells
   * mapping neighbor cell Id with {#late,#early, and #wrong}
   */
  std::map<uint16_t,LteEnbDSONSapUser::HandoverIssueInfo> m_hoIssueInfo;

  void SaveCIOValuesToLog(); //For algorithm 2
  bool m_cioValuesFirstWrite;
  std::map<uint8_t, int8_t> m_cioList;
//  bool m_MroAlgorithm2Enable; //Adaptive CIO
//  void MroAlgorithm2(void);
  void SetHandoverCIOParams();
  bool m_hoInfoFirstWrite;

  /*
   * SON interface
   */
  LteEnbDSONSapProvider* m_dSONSapProvider;
//  Ptr<LteEnbDSONSapProvider> m_dSONSapProvider; //Avoid memory leak
  LteEnbDSONSapProvider* GetLteEnbDSONSapProvider(void);

  LteEnbDSONSapUser* m_dSONSapUser;
  //  Ptr<LteEnbDSONSapUser> m_dSONSapUser;
  void SetLteEnbDSONSapUser(LteEnbDSONSapUser* dSONUser);

  void EnbRrcRcvPMReportNotify(SONCommon::SONMsgInfo msgInfo);//Receive request from DSON
  /*
   * Enb RRC received Updated Parameters
   */
  void EnbRrcRecvUpdateParamConfig(SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList);
  /*
   *Enb RRC receives DSONParam.Request from Epc Application
   *Forward to DSON for translation
   */
  void EnbRrcRecvDSONParamReq(SONCommon::SONMsgInfo msgInfo);
  void EnbRrcRecvDSONParamResp(SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList);
  void EnbRrcSetMroParams(LteEnbDSONSapProvider::MroParameters params);
  void RrcSetMroParams(bool updateReq, LteEnbDSONSapProvider::MroParameters params);
  void RrcSetA3A4Params(LteEnbDSONSapProvider::MroParameters params, uint8_t A4thresh);
  /* Assume that, serving cell knows all the neighboring cell, the target is to manage CIO utilization at CSON
   * which CIO is changed, so we have to detect active cell for CIO management at CSON (a trick likes ANR but not real)
   * Next Version will utilize ANR functions in lte-anr.cc
   */
//  std::list<uint16_t> m_newActiveCell;

  /*
   * Simple Minimization Drive Tests
   * In near future, MDT features and signaling may be implemented
   */
  bool m_enableMDT;
  std::set<uint8_t> m_mdtMeasIDs;
  uint64_t m_mdtInterval;
  uint8_t AddUeMeasReportConfigForMDT ();
  std::map<uint16_t, std::queue<Ptr<Packet> > > m_uePacketBuffer;
  double m_cioResol; //Resolution of CIO 1, 0.5, 0.25
