...
//      double sum = 0;
//      for (Values::const_iterator it1 = (*m_txPsd).ConstValuesBegin();
//    		  it1 != (*m_txPsd).ConstValuesEnd(); ++it1) {
//    	  sum += (*it1);
//      }
//      NS_LOG_UNCOND("LteSpectrumPhy::StartTxDataFrame SUM = " << sum);
...
//  NS_LOG_UNCOND ("LteSpectrumPhy::StartRx state: " << m_state);
  ...
  if (lteDataRxParams != 0)
  ...
  else
    {
	  ...
	  NS_LOG_LOGIC("LteSpectrumPhy::StartRx Other Signals");
      m_interferenceData->AddSignal (rxPsd, duration);
      m_interferenceCtrl->AddSignal (rxPsd, duration);
    }    
}
...
{
  NS_LOG_LOGIC ("LteSpectrumPhy::StartRxData state " << m_state);
  switch (m_state)
    {
      case TX_DATA:
      case TX_DL_CTRL:
      ...
      case RX_DATA:
        ...
        // simultaneously (e.g., at the eNB).
        {
          ...
          if (params->cellId  == m_cellId)
            {
              NS_LOG_DEBUG ("LteSpectrumPhy::StartRxData synchronized with this signal (cellId=" << params->cellId << ")");
              ...
              if (params->packetBurst)
                {
                  ...
                  NS_LOG_DEBUG("LteSpectrumPhy::StartRxData packetBurst");
                  m_interferenceData->StartRx (params->psd);
                  
                  m_phyRxStartTrace (params->packetBurst);
                }
                NS_LOG_DEBUG (this << " insert msgs " << params->ctrlMsgList.size ());
              m_rxControlMessageList.insert (m_rxControlMessageList.end (), params->ctrlMsgList.begin (), params->ctrlMsgList.end ());//(position of the list, begin of info, last of info)
              
              NS_LOG_LOGIC (this << " numSimultaneousRxEvents = " << m_rxPacketBurstList.size ());
            }
          else
            {
              NS_LOG_DEBUG ("LteSpectrumPhy::StartRxData not in sync with this signal (cellId="
