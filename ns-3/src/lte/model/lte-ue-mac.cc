...
{
  //NS_LOG_FUNCTION (this);
  //NS_LOG_DEBUG("LteUeMac::SendReportBufferStatus RNTI " << m_rnti);
  if (m_rnti == 0)
    {
  ...
  if (m_ulBsrReceived.size () == 0)//Uplink Data Buffer
    {
  ...
  std::map <uint8_t, LteMacSapProvider::ReportBufferStatusParameters>::iterator it;  
  std::vector<uint32_t> queue (4, 0); // one value per each of the 4 Logical Channel Groups (LCGs), 0 1 2 3
  //LCG 0: SRB1, SRB2 (signaling radio bearer)
  //LCG 1: GBR data radio bearer
  //LCG 2: Non-GBR data radio bearer

  for (it = m_ulBsrReceived.begin (); it != m_ulBsrReceived.end (); it++)
    {
      uint8_t lcid = it->first;
      //NS_LOG_DEBUG("m_ulBsrReceived contains lcid " << (uint32_t)lcid);
...
{
  NS_LOG_DEBUG ("LteUeMac::DoAddLc" << " rnti " << m_rnti << " lcId " << (uint32_t) lcId);
  NS_ASSERT_MSG (m_lcInfoMap.find (lcId) == m_lcInfoMap.end (), "cannot add channel because LCID " << (uint32_t)lcId << " is already present");
...
//	  NS_LOG_DEBUG ("LteUeMac::m_lcInfoMap contain " << " lcId " << (uint32_t)(*it).first);
//	  it++;
//  }
//  /*****************************/
}
...
{
  NS_LOG_DEBUG ("LteUeMac::DoRemoveLc"  << " rnti " << m_rnti << " lcId " << (uint32_t)lcId);
  NS_ASSERT_MSG (m_lcInfoMap.find (lcId) != m_lcInfoMap.end (), "could not find LCID " << (uint32_t)lcId);
...
LteUeMac::DoReset ()
{
  NS_LOG_DEBUG ("LteUeMac::DoReset " << "RNTI " << m_rnti);
  std::map <uint8_t, LcInfo>::iterator it = m_lcInfoMap.begin ();
  while (it != m_lcInfoMap.end ())
    {
      // don't delete CCCH)
      ...
      if (dci.m_ndi == 1)
        {
          ...
          if (activeLcs == 0)
            {
        	  //THANG NS_LOG_ERROR
              NS_LOG_INFO("LteUeMac::DoReceiveLteControlMessage " << " No active flows for this UL-DCI");
              return;
            }
          ...
          if ((statusPduMinSize != 0)&&(bytesPerActiveLc < statusPduMinSize))
            {
              ...
              NS_LOG_INFO (this << " Reduced resource -> send only Status, b ytes " << statusPduMinSize);
          ...
          for (it = m_lcInfoMap.begin (); it != m_lcInfoMap.end (); it++)
            {
              ...
              NS_LOG_INFO (this << " Processing LC " << (uint32_t)(*it).first << " bytesPerActiveLc " << bytesPerActiveLc);
                      ...
                      if (((*itBsr).second.statusPduSize > 0) && (bytesForThisLc > (*itBsr).second.statusPduSize))
                        {
                          ...
                          NS_LOG_INFO (this << " serve STATUS " << (*itBsr).second.statusPduSize);
                          (*itBsr).second.statusPduSize = 0;
                        }
                      else
                        {
                      ...
                      if ((bytesForThisLc > 7)    // 7 is the min TxOpportunity useful for Rlc
                              ...
                              || ((*itBsr).second.txQueueSize > 0)))
                        {
                          if ((*itBsr).second.retxQueueSize > 0)
                            {
                              NS_LOG_INFO (this << " serve retx DATA, bytes " << bytesForThisLc);
                          ...
                          else if ((*itBsr).second.txQueueSize > 0)
                            {
                              ...
                              NS_LOG_INFO (this << " serve tx DATA, bytes " << bytesForThisLc << ", RLC overhead " << rlcOverhead);
                              (*it).second.macSapUser->NotifyTxOpportunity (bytesForThisLc, 0, 0);
      ...
      else
        {
          ...
          NS_LOG_INFO (this << " UE MAC RETX HARQ " << (uint16_t)m_harqProcessId);
...
LteUeMac::DoSubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  ...
  if ((Simulator::Now () >= m_bsrLast + m_bsrPeriodicity) && (m_freshUlBsr == true))
    {
      SendReportBufferStatus (); //cancel during RLF, re-start after RRC connection?
