...
private:
  ...
  /**************THANG***************/
  void SaveUeMeasurements (uint16_t cellId, double rsrp, double rsrq, bool useLayer3Filtering, bool newMeas, double servRSRP);

  void SaveUeMeasurementsBeforeRLF ();
  /**
...
/**
   * RLF parameters
   */
uint8_t m_outofsyncCnt;
uint8_t m_insyncCnt;
uint8_t m_N310; //Max consecutive #out-of-sync indications
Time   m_T310; // (ms) Timer for RLF recovery
uint8_t m_N311; //Max consecutive #in-sync indications
double m_Qoutofsync;
double m_Qinsync;
EventId m_T310Timeout;
uint8_t m_measurementMetric; //0: RSRP; 1: RSRQ; 2: SINR
//double m_rsSINR;

//TracedCallback<uint64_t, uint16_t, uint16_t, double, bool> m_RLFDetectionTrace;
//typedef void (* RLFDetectionCallback)
//		(const uint64_t cellId, const uint64_t imsi, const uint16_t rnti, const double rsrq, const bool rlfHappen);

void SINRCalculation();
void ReportRLFDetection(uint16_t cellId, uint64_t imsi, uint16_t rnti, double rsrq, bool rlfHappen);
void RSRQTracing(uint16_t cellId, uint64_t imsi, uint16_t rnti, double rsrq, double rsrp);
bool m_RLFFirstWrite;
bool m_RSRQFirstWrite;
bool m_enTracePos;//Enable generating position
struct RLFMeasurements
{
	double m_rsrq;  // [dB]
	bool m_rlfhappen;
};
RLFMeasurements m_rlfMeas;

bool m_MeasurementFirstWrite;
//bool m_rlfMeasFirstWrite;
bool m_enTraceMeas;
bool m_enTraceRLFData;
void TraceALlMeasurements(uint16_t cellId, uint64_t imsi, uint16_t rnti, double rsrp, double rsrq, bool nextMeas);
bool m_rlfdetect; //rlf status
uint16_t m_rlfRNTI;
uint16_t m_rlfCellid;
EventId m_t301;
Time m_T301Duration;
void ConnectionReestablishmentRequestTimeout (uint64_t imsi, uint16_t rnti);
void RLFTrigger(uint16_t cellId, uint16_t rnti);
/*
 * Trace the difference of received signals among serving cell and neighbor cells
 */
void TraceMeasurementsDifference(uint16_t cellId, uint64_t imsi, uint16_t rnti, double diff, bool nextMeas);
EventId m_mdtEvent;
uint8_t m_mdtMeasId;
Time m_mdtReportInterval;
uint8_t m_eventA4MeasId;
void SendMeasurementReportForMDT();
