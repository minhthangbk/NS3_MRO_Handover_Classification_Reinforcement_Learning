...
private:
  ...
  /**************THANG***************/
  void SaveUeMeasurements (uint16_t cellId, double rsrp, double rsrq, bool useLayer3Filtering, bool newMeas);
  /**
...
/**
   * RLF parameters
   */
int8_t m_outofsyncCnt;
int8_t m_insyncCnt;
int8_t m_N310; //Max consecutive #out-of-sync indications
Time   m_T310; // (ms) Timer for RLF recovery
int8_t m_N311; //Max consecutive #in-sync indications
EventId m_T311Timeout;
...
void RLFRecoveryTimeout();
void ReportRLFDetection(uint16_t cellId, uint64_t imsi, uint16_t rnti, double rsrq, bool rlfHappen);
void RSRQTracing(uint16_t cellId, uint64_t imsi, uint16_t rnti, double rsrq);
