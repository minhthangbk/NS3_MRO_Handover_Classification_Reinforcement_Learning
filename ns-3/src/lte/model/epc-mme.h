...
public:
  /*
   * Interface with CSON Entity (OAM layer)
   */
  EpcMmeCSONSapProvider* m_CSONSapProvider;
  EpcMmeCSONSapUser* m_CSONSapUser;
  void SetEpcMmeCSONSapUser(EpcMmeCSONSapUser* sapUser);
  EpcMmeCSONSapProvider* GetEpcMmeCSONSapProvider();
  /*
   * Interface with EpcEnbApplication
   * Receiving Handover Update from eNB
   */
  void MmeRcvPMReportNotify(uint16_t cellId, SONCommon::SONMsgInfo msgInfo, SONCommon::PerformanceReport pmReport);
  void MmeRecvDSONParamResp(uint16_t cellId, SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList);
  /*
   * Mme receives updated parameters from CSON, and forward to DSON
   */
  void MmeRecvUpdateParamConfig(uint16_t cellId, SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList);
  /*
   * Mme receives DSON Parameters Request MSG from CSON
   * Then it broadcast the request to all eNB in the list
   */
  void MmeRecvDSONParamReq(SONCommon::SONMsgInfo msgInfo);
