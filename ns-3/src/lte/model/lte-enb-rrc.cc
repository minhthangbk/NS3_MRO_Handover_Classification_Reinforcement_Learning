...
UeManager::DoInitialize ()
{
	...
	//parent Enb RRC Entity add the Ue
	m_rrc->m_cmacSapProvider->AddUe (m_rnti);
	m_rrc->m_cphySapProvider->AddUe (m_rnti);
	// setup logical channel of SRB0 at parent eNB
	{
		uint8_t lcid = 0;
		...
		m_rrc->m_cmacSapProvider->AddLc (lcinfo, rlc->GetLteMacSapUser ()); //LC for signaling
	}
...
TypeId UeManager::GetTypeId (void)
{
  static TypeId  tid = TypeId ("ns3::UeManager")
    ...
    .AddAttribute ("DataRadioBearerMap", "List of UE DataRadioBearerInfo by DRBID.",
                   ObjectMapValue (),
                   MakeObjectMapAccessor (&UeManager::m_drbMap),//for downcast in main, by using GetAttribute
...
UeManager::GetDataRadioBearerMapping(){
	return m_drbMap;
}
/*****************************************/
void
UeManager::SetupDataRadioBearer (EpsBearer bearer, uint8_t bearerId, uint32_t gtpTeid, Ipv4Address transportLayerAddress)
{
	...
	NS_ASSERT_MSG ( bearerId == 0 || bid == bearerId, "bearer ID mismatch (" << (uint32_t) bid << " != "
			<< (uint32_t) bearerId << ", the assumption that ID are allocated in the same way by MME and RRC is not valid any more");
	drbInfo->m_epsBearerIdentity = bid;
	drbInfo->m_drbIdentity = drbid;
	drbInfo->m_logicalChannelIdentity = lcid;

	if ((m_state == HANDOVER_JOINING) || (m_state == INITIAL_RANDOM_ACCESS))
	{
	...
	lcinfo.gbrDl = bearer.gbrQosInfo.gbrDl; //LC for DATA
...
{
	NS_LOG_FUNCTION ("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::RecordDataRadioBearersToBeStarted");
	for (std::map <uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it = m_drbMap.begin ();
			it != m_drbMap.end ();
			++it)
	{
...
{
	NS_LOG_FUNCTION ("CellId: " << m_rrc->m_cellId << " RNTI: " << (uint32_t) m_rnti << " UeManager::StartDataRadioBearers");
...
{
	NS_LOG_FUNCTION ("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti <<" UeManager::ReleaseDataRadioBearer: " << (uint32_t) m_rnti << (uint32_t) drbid);
	uint8_t lcid = Drbid2Lcid (drbid);
...
{
	NS_LOG_DEBUG("LteEnbRrc::DoSendReleaseDataRadioBearer " << " RNTI " << rnti << " bearerId " << bearerId << "cellId " << m_cellId);
	Ptr<UeManager> ueManager = GetUeManager (rnti);
	// Bearer de-activation towards UE
	ueManager->ReleaseDataRadioBearer (bearerId);//command from Enb to Ue (RLF disconnection donot have this command)
...
void
LteEnbRrc::RemoveUeDueToRLF(uint16_t rnti)
{
	RemoveUe(rnti);
}

void
LteEnbRrc::RemoveUeDueToConnectionReestablishmentReject(uint16_t rnti){
	NS_LOG_DEBUG ("LteEnbRrc::RemoveUeDueToConnectionReestablishmentReject RNTI " << (uint32_t) rnti << " cellId " << m_cellId);
	RemoveUe(rnti);
}
/*******************************************************/
...
{
	NS_LOG_FUNCTION ("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::ScheduleRrcConnectionReconfiguration");
	switch (m_state)
	{
	case INITIAL_RANDOM_ACCESS:
...
{
	NS_LOG_FUNCTION ("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::PrepareHandover");
	switch (m_state)
	{
	case CONNECTED_NORMALLY:
	{
...
{
	NS_LOG_FUNCTION ("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::RecvHandoverRequestAck");
...
{
	NS_LOG_FUNCTION ("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::GetRadioResourceConfigForHandoverPreparationInfo");
...
{
	NS_LOG_FUNCTION ("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::GetErabList");
	std::vector<EpcX2Sap::ErabToBeSetupItem> ret;
	for (std::map <uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it =  m_drbMap.begin ();
			it != m_drbMap.end ();
			++it)
	{
		NS_LOG_DEBUG ("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::GetErabList QoS: " << it->second->m_epsBearer.qci);
...
{
	NS_LOG_FUNCTION ("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::RecvRrcConnectionRequest");
	switch (m_state)
	{
	case INITIAL_RANDOM_ACCESS:
	{
...
{
	NS_LOG_FUNCTION ("CellId: "<< m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::RecvRrcConnectionReconfigurationCompleted");
	switch (m_state)
	{
	case CONNECTION_RECONFIGURATION:
		StartDataRadioBearers ();
	...
	case CONNECTED_NORMALLY:
		...
		NS_LOG_DEBUG ("UeManager::RecvRrcConnectionReconfigurationCompleted ignore msg");
		break;

	case HANDOVER_LEAVING:
		NS_LOG_INFO ("UeManager::RecvRrcConnectionReconfigurationCompleted in state " << ToString (m_state));
		break;
	...
	case HANDOVER_JOINING:
	{
		...
		for (std::map <uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it =  m_drbMap.begin ();//Copy all bearer's Info
				it != m_drbMap.end ();
				++it)
		{
...
void
UeManager::RecvRrcConnectionReestablishmentRequest (LteRrcSap::RrcConnectionReestablishmentRequest msg, bool validUeContext)
{
	NS_LOG_FUNCTION("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::RecvRrcConnectionReestablishmentRequest "
			<< "validUeContext " << validUeContext);
	switch (m_state)
	{
	case CONNECTED_NORMALLY:
		break;
	...
	case INITIAL_RANDOM_ACCESS:
		m_connectionRequestTimeout.Cancel();
		//to avoid connection request timeout, because the at the INITIAL_RANDOM_ACCESS, Ue starts waiting for this message
		//So the timer has to be stopped
		break;
		/*************************************/
	...
	if(!validUeContext)
	{//invalid Ue Context
		LteRrcSap::RrcConnectionReestablishmentReject RrcConReeRej;
		m_rrc->m_rrcSapUser->SendRrcConnectionReestablishmentReject (m_rnti, RrcConReeRej);
		SwitchToState (CONNECTED_NORMALLY);
	}
	else
	{//valid Ue context
		m_imsi = msg.ueIdentity.imsi;
		if (m_rrc->m_s1SapProvider != 0)
		{
			m_rrc->m_s1SapProvider->InitialUeMessage (m_imsi, m_rnti);
		}
		...
		RecordDataRadioBearersToBeStarted();
		m_connectionReestablishmentTimeout = Simulator::Schedule(m_rrc->m_connectionReestablishmentTimeoutDuration,
				&LteEnbRrc::ConnectionReestablishmentTimeout,m_rrc,m_rnti);
...
UeManager::RecvMeasurementReport (LteRrcSap::MeasurementReport msg)
{
			...
			&& (m_rrc->m_handoverMeasIds.find (measId) != m_rrc->m_handoverMeasIds.end ()))
	{
		...
		m_rrc->m_handoverManagementSapProvider->ReportUeMeas (m_rrc->m_cellId, m_imsi, m_rnti, msg.measResults);//m_rrc->m_cellId, m_rnti, msg.measResults
	}
	...
	if(m_rrc->m_mdtMeasIDs.find (measId) != m_rrc->m_mdtMeasIDs.end ()){
		NS_LOG_DEBUG("UeManager::RecvMeasurementReport Periodical MeasurementReport for MDT cellId " << m_rrc->m_cellId);
	}

	// fire a trace source for this measurement
...
{
	NS_LOG_FUNCTION("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::AddDataRadioBearerInfo QoS: " << drbInfo->m_epsBearer.qci);
...
{
	NS_LOG_FUNCTION ("UeManager::GetDataRadioBearerInfo: " << (uint32_t) drbid);
	NS_ASSERT (0 != drbid);
	std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it = m_drbMap.find (drbid);
...
{
	NS_LOG_FUNCTION("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::BuildRrcConnectionReconfiguration");
	LteRrcSap::RrcConnectionReconfiguration msg;
...
{
	LteRrcSap::RadioResourceConfigDedicated rrcd;
	NS_LOG_FUNCTION("CellId: " << m_rrc->m_cellId << " RNTI: " << m_rnti << " UeManager::BuildRadioResourceConfigDedicated");
	if (m_srb1 != 0)
	{
	...
	for (std::map <uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it = m_drbMap.begin ();
			it != m_drbMap.end ();
			++it)
	{//All users have similar QoS
		LteRrcSap::DrbToAddMod dtam;
		dtam.epsBearer = it->second->m_epsBearer;
...
{
  NS_LOG_LOGIC("UeManager::SwitchToState " << ToString (newState) << " from " << ToString(m_state));
    ...
    m_reconfigureUes (false)
{
  ...
  /*
   * DSON interface set up
   */
  m_dSONSapProvider = new MemberLteEnbDSONSapProvider<LteEnbRrc>(this);
  m_dSONSapUser = NULL;

//  m_UeCxtTimerForEHO = Seconds(3); //
//  m_UeCxtTimerForWHO = Seconds(3);
//  m_NumTooEarlyHO = 0;
//  m_NumTooLateHO = 0;
//  m_NumWrongCellHO = 0;
//  m_NumPingpongHO = 0;
//  m_NumHOs = 0;
  ...
  m_curMeasConfigIdHO = 0;
  m_curMeasConfigIdAnr = 0;
  m_curHysVal = 0;
  m_curA3OffVal = 0;
  m_curTTTVal = 0;

  m_A3TTTFirstWrite = true; //Trace a3, TTT
  m_cioValuesFirstWrite = true;
...
//  m_aggSq = 0; //Aggregated variance
//  m_aggSum = 0; //Aggregated variance
//  m_aggNumHO = 0;
  Simulator::Schedule(MilliSeconds(10),&LteEnbRrc::AddUeMeasReportConfigForMDT,this);
}
...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
    ...
    .AddAttribute ("SystemInformationPeriodicity",
                   ...
                   MakeTimeChecker ())
    // SRS related attributes ~ maximum number of UEs
    ...
    .AddAttribute ("ConnectionSetupTimeoutDuration",
                   ...
                   MakeTimeChecker ())
    .AddAttribute ("ConnectionReestablishmentTimeoutDuration",
            "After accepting connection request, if no RRC CONNECTION REESTABLISHMENT"
            "COMPLETE is received before this time, the UE "
            "context is destroyed.",
            TimeValue (MilliSeconds (150)),
            MakeTimeAccessor (&LteEnbRrc::m_connectionReestablishmentTimeoutDuration),
            MakeTimeChecker ())
    ...
    .AddTraceSource ("RecvA3MeasReport",
    				"THANG adds trace for the a3 measurement report",
					MakeTraceSourceAccessor (&LteEnbRrc::m_recvA3MeasReportCallback),
					"ns3::LteEnbRrc::ReceiveA3MeasReport")
	.AddTraceSource ("RecvA4MeasReport",
					"THANG adds trace for the a4 measurement report",
					MakeTraceSourceAccessor (&LteEnbRrc::m_recvA4MeasReportCallback),
					"ns3::LteEnbRrc::ReceiveA4MeasReport"
					)
	.AddTraceSource ("RecvMroStatData",
					"tracing MRO statistical data",
					MakeTraceSourceAccessor(&LteEnbRrc::m_recvMroStatData),
					"ns3::LteEnbRrc::ReceiveMroStatisticalData")
    .AddAttribute("Tuecxtstore",
            		"Timer for UE Context Store",
					TimeValue(Seconds(1)),
					MakeTimeAccessor (&LteEnbRrc::SetUeCxtStoreTimer,&LteEnbRrc::GetUeCxtStoreTimer),
					MakeTimeChecker ())
	.AddAttribute ("MDTEnable",
		    		"All UEs in serving cell are MDT devices",
					BooleanValue (false),
					MakeBooleanAccessor (&LteEnbRrc::m_enableMDT),
					MakeBooleanChecker ())
	.AddAttribute ("MDTInterval",
					"Report Interval specified in 36.331",
					UintegerValue (480),
					MakeUintegerAccessor (&LteEnbRrc::m_mdtInterval),
					MakeUintegerChecker <uint64_t>())
	.AddAttribute ("CIOResolution",
					"1, 0.5., 0.25",
					DoubleValue (1),
					MakeDoubleAccessor (&LteEnbRrc::m_cioResol),
...
{
	NS_LOG_FUNCTION ("LteEnbRrc::GetUeManager " << (uint32_t) rnti);
	NS_ASSERT (0 != rnti);

	std::map<uint16_t, Ptr<UeManager> >::iterator it = m_ueMap.find (rnti);
	if(it == m_ueMap.end()){
		//rnti not found
		NS_LOG_UNCOND("LteEnbRrc::GetUeManager UeManager not found with rnti " << rnti);
		return NULL;
	}
	return it->second;
}
...
LteEnbRrc::AddUeMeasReportConfig (LteRrcSap::ReportConfigEutra config)
{
	...
	{
		NS_LOG_LOGIC ("AddUeMeasReportConfig may not be called after the simulation has run cellId " << m_cellId);
	}
	...
	NS_LOG_LOGIC("LteEnbRrc::AddUeMeasReportConfig cellId " << m_cellId << " reportConfig.reportConfigEutra.triggerType "
			<< (uint32_t) reportConfig.reportConfigEutra.triggerType << " reportConfig.reportConfigId " << (uint32_t)reportConfig.reportConfigId);
	// create the measurement identity
	LteRrcSap::MeasIdToAddMod measId;
	measId.measId = nextId; //an index that map reportConfigID with measObjectID
	measId.measObjectId = 1;//Only 1 measurement object~1 down link frequency
	measId.reportConfigId = nextId; //because there is only 1 measObject, so measId.measId and measId.reportConfigId are increased together
	...
	NS_LOG_LOGIC("LteEnbRrc::AddUeMeasReportConfig  cellId " << m_cellId << " measIdToAddModList size() "
			<< (uint32_t)m_ueMeasConfig.measIdToAddModList.size());
	return nextId;
}
		...
		uint16_t ulEarfcn, uint16_t dlEarfcn, uint16_t cellId)
{
	...
	m_cmacSapProvider->ConfigureMac (ulBandwidth, dlBandwidth, cellId);
...
{
	NS_LOG_INFO ("LteEnbRrc::SendData " << packet);
	Ptr<Packet> packetCopy = packet->Copy();
	EpsBearerTag tag;
	bool found = packet->RemovePacketTag (tag);
	NS_ASSERT_MSG (found, "no EpsBearerTag found in packet to be sent");

	std::map<uint16_t, Ptr<UeManager> >::iterator it = m_ueMap.find (tag.GetRnti ());
	if(it == m_ueMap.end ())
	{ //if no result
		return false;
	}
//	if((m_uePacketBuffer.find(it->second->GetRnti()) != m_uePacketBuffer.end()))
//	{
//		if(m_uePacketBuffer[it->second->GetRnti()].size() > 0)
//		{
//			m_uePacketBuffer[it->second->GetRnti()].pop();
//		}
//	}
//	m_uePacketBuffer[it->second->GetRnti()].push(packetCopy);
	Ptr<UeManager> ueManager = it->second;
	ueManager->SendData (tag.GetBid (), packet);
	return true; //for error model
}
...
{
	NS_LOG_UNCOND ("LteEnbRrc::ConnectionRequestTimeout RNTI" << rnti << " cellId " << m_cellId);
...
{
	NS_LOG_UNCOND ("LteEnbRrc::ConnectionSetupTimeout RNTI " << rnti << " cellId " << m_cellId);
...
{
	NS_LOG_LOGIC ("LteEnbRrc::HandoverLeavingTimeout RNTI " << rnti << " cellId " << m_cellId);
...
void
LteEnbRrc::ConnectionReestablishmentTimeout(uint16_t rnti){
	NS_ASSERT_MSG (GetUeManager (rnti)->GetState () == UeManager::CONNECTION_REESTABLISHMENT,
			"HandoverLeavingTimeout in unexpected state " << ToString (GetUeManager (rnti)->GetState ()));
	RemoveUe (rnti);
}
...
{
	NS_LOG_DEBUG ("LteEnbRrc::SendHandoverRequest RNTI " << rnti << " cellId " << m_cellId);
...
{
	NS_LOG_LOGIC ("LteEnbRrc::DoCompleteSetupUe RNTI " << rnti << "cellId " << m_cellId);
...
{
	NS_LOG_DEBUG ("LteEnbRrc::DoRecvRrcConnectionRequest RNTI " << rnti << " cellId " << m_cellId);
...
{
	NS_LOG_DEBUG ("LteEnbRrc::DoRecvRrcConnectionSetupCompleted RNTI " << rnti << " cellId " << m_cellId);
...
{
	NS_LOG_DEBUG ("LteEnbRrc::DoRecvRrcConnectionReconfigurationCompleted " << "cellId " << m_cellId << " RNTI " << rnti);
	/*
	 * Fix Error: RrcConnectionReconfiguration Failure (maybe)
	 * 2015-10-30
	 */
	Ptr<UeManager> uePtr = GetUeManager (rnti);
	if(uePtr != NULL){
		uePtr->RecvRrcConnectionReconfigurationCompleted (msg);
	}else{
		NS_LOG_DEBUG("RrcConnectionReconfiguration Failure ");
	}
...
{
	bool validUeContext = true;
	bool ueContextFound = false;
	for(std::list<HandOverUeContext>::iterator it = m_UeContextForEHO.begin(); (it != m_UeContextForEHO.end()) && (!ueContextFound); it++)
	{
		if((it->m_rnti == msg.ueIdentity.cRnti) && (it->m_cellId == msg.ueIdentity.physCellId)
				&& (msg.reestablishmentCause == LteRrcSap::OTHER_FAILURE))
		{
			ueContextFound = true;
			it->m_ehoDetected = true;
			it->m_rlfMeasData = msg.measData;
		}
	}

	GetUeManager(rnti)->SetImsi(msg.ueIdentity.imsi);
	GetUeManager (rnti)->RecvRrcConnectionReestablishmentRequest (msg,validUeContext);
	if(msg.ueIdentity.physCellId != m_cellId)
	{//Handover failure due to RLF
		EpcX2SapProvider::RlfIndicationMessageParams rlfMsgParams;
		rlfMsgParams.failureCRNTI = msg.ueIdentity.cRnti;
		rlfMsgParams.failureIMSI = msg.ueIdentity.imsi;
		rlfMsgParams.failureCellId = msg.ueIdentity.physCellId;
		rlfMsgParams.reportCellId = m_cellId;
		rlfMsgParams.forwarded = false;
		rlfMsgParams.sourceCellId = msg.ueIdentity.physCellId;
		for(std::list<LteRrcSap::MeasResultEutra>::iterator measIt=msg.measData.begin(); measIt!=msg.measData.end(); ++measIt)
		{
			//TODO: Add measurement data for RLF Msg
			//(This is novel method. In fact, eNB request UEs to send logged RLF data)
			rlfMsgParams.rlfMeasData.push_back(*measIt);
		}

		uint32_t newTeId = 0;
		for(std::map<uint32_t, X2uTeidInfo>::iterator x2uTeIdIt = m_x2uTeidInfoMap.begin();
				x2uTeIdIt != m_x2uTeidInfoMap.end(); x2uTeIdIt++)
		{
			if(x2uTeIdIt->second.rnti == rnti)
			{
				newTeId = x2uTeIdIt->first;
			}
		}
		NS_ASSERT_MSG(newTeId > 0,"CellId: " <<m_cellId << " Invalid RNTI: " << rnti);
		rlfMsgParams.newTeId = newTeId;
		m_x2SapProvider->SendRlfIndicationMessage(rlfMsgParams);
	}
	else
	{//Connection drop
		//Send to trace sink at main
		SONCommon::MROProblemInfo proInfo;
		proInfo.cellId = m_cellId; //neighbor cell Id params.failureIMSI
		proInfo.imsi = msg.ueIdentity.imsi;
		proInfo.problem = SONCommon::RLF_CONNECTION_DROP;
		m_recvMroStatData(m_cellId,proInfo);
	}
}
...
{
	NS_LOG_DEBUG ("LteEnbRrc::DoRecvRrcConnectionReestablishmentComplete RNTI " << rnti << "cellId " << m_cellId);
...
{
	NS_LOG_DEBUG("LteEnbRrc::DoRecvMeasurementReport m_cellId " << m_cellId << " rnti " << rnti << " RSRP " <<(uint16_t)msg.measResults.rsrpResult);
	std::map<uint16_t, Ptr<UeManager> >::iterator it = m_ueMap.find (rnti);
	if(it != m_ueMap.end ()){ //if found
		Ptr<UeManager> ueManager = it->second;

		//THANG adds Trace source for A4 threshold algorithm (as HASAN-MLB)
		if ((m_handoverManagementSapProvider != 0) && (m_handoverMeasIds.find (msg.measResults.measId) != m_handoverMeasIds.end ()))
		{
			m_recvA3MeasReportCallback(m_cellId, it->second->m_imsi, rnti, msg.measResults);
		}

		//THANG adds trace source for collect candidate target cell based on A4 measurement report
		if ((m_anrSapProvider != 0)
				&& (m_anrMeasIds.find (msg.measResults.measId) != m_anrMeasIds.end ()))
		{
			m_recvA4MeasReportCallback(m_cellId, rnti, msg);
		}
...
{
	NS_LOG_FUNCTION("CellId: " << m_cellId << " DoDataRadioBearerSetupRequest RNTI: " << request.rnti << " Bearer QoS: " << request.bearer.qci);
...
{
	NS_LOG_DEBUG("LteEnbRrc::DoPathSwitchRequestAcknowledge " << params.rnti << "cellId " << m_cellId);
...
{
	//NS_LOG_FUNCTION (this);
	NS_LOG_FUNCTION("CellId: " << m_cellId << " LteEnbRrc::DoRecvHandoverRequest from CellId: " << req.sourceCellId << " RNTI: " << req.oldEnbUeX2apId);
	...
	for(std::list<HandOverUeContext>::reverse_iterator rit = m_UeContextForEHO.rbegin();
			(rit != m_UeContextForEHO.rend()) && (!pingpongFound); ++rit){
		if((rit->m_imsi == req.mmeUeS1apId) && (rit->m_cellId == req.sourceCellId)){
			pingpongFound = true; //safely escape from loop
			rit->m_ppDetected = true;
		}
	}
	...
	uint16_t rnti = AddUe (UeManager::HANDOVER_JOINING); //Create new UeManager entity for new RNTI
	...
	{
		NS_FATAL_ERROR("LteEnbRrc::DoRecvHandoverRequest failed to allocate a preamble for non-contention based RA => cannot accept HO");
		RemoveUe (rnti);
		return;
	}
	...
	ueManager->SetImsi ((uint64_t)req.mmeUeS1apId);
	...
	for (std::vector <EpcX2Sap::ErabToBeSetupItem>::iterator it = req.bearers.begin ();
			...
			++it)
	{
		//NS_LOG_DEBUG("CellId: " << m_cellId << " RNTI: " << rnti << " ueManager->SetupDataRadioBearer: " << (uint32_t)it->erabLevelQosParameters.qci);
	...
	HandOverUeContext hoUeCxt;
	hoUeCxt.m_cellId = ackParams.sourceCellId;
	hoUeCxt.m_rnti = ackParams.newEnbUeX2apId;
	hoUeCxt.m_imsi = ackParams.imsi;
	hoUeCxt.m_ehoDetected = false;
	hoUeCxt.m_whoDetected = false;
	hoUeCxt.m_ppDetected = false;
	m_UeContextForWHO.push_back(hoUeCxt);
	Simulator::Schedule(m_UeCxtTimerForWHO,&LteEnbRrc::RemoveHandoverdUeContextForWHO,this,hoUeCxt);
...
{
	NS_LOG_DEBUG ("LteEnbRrc::DoRecvHandoverRequestAck RNTI " << params.oldEnbUeX2apId << " sourceCellId " << params.sourceCellId
			<< " targetCellId " << params.targetCellId << " new RNTI " << params.newEnbUeX2apId);
	...
	//Get and save new RNTI for new UeManager Entity
	LteEnbRrc::HandOverUeContext hoUeContext;
	hoUeContext.m_rnti=  params.newEnbUeX2apId;
	hoUeContext.m_cellId = params.targetCellId;
	hoUeContext.m_imsi = params.imsi;
	hoUeContext.m_ehoDetected = false;
	hoUeContext.m_whoDetected = false;
	hoUeContext.m_ppDetected = false;
	m_UeContextForEHO.push_back(hoUeContext);
	NS_LOG_LOGIC ("LteEnbRrc::DoRecvHandoverRequestAck m_UeContextForEHO add RNTI " << hoUeContext.m_rnti << " targetCellId " << params.targetCellId);
	//assume the buffer is big enough for EHO detection, and no remove
	Simulator::Schedule(m_UeCxtTimerForEHO,&LteEnbRrc::RemoveHandoverdUeContextForEHO,this,hoUeContext);

	uint16_t rnti = params.oldEnbUeX2apId;
	Ptr<UeManager> ueManager = GetUeManager (rnti); //Access the old UeManager entity to modify and then delete it
...
{
	NS_LOG_DEBUG ("LteEnbRrc::DoRecvSnStatusTransfer RNTI " << params.newEnbUeX2apId << " cellId " << m_cellId);
...
{
	NS_LOG_DEBUG ("LteEnbRrc::DoRecvUeContextRelease old RNTI " << params.oldEnbUeX2apId << " cellId " << m_cellId);
...
LteEnbRrc::DoRecvUeData (EpcX2SapUser::UeDataParams params)
{
	...
	{
		//If the User has RLF before and reconnect to this cell???
...
{
	NS_LOG_DEBUG("LteEnbRrc::DoRrcConfigurationUpdateInd RNTI " << cmacParams.m_rnti << " cellId " << m_cellId);
...
LteEnbRrc::DoAddUeMeasReportConfigForHandover (LteRrcSap::ReportConfigEutra reportConfig)
{
	...
	m_curMeasConfigIdHO = measId; //save this id for modifying setting later
	m_curHysVal = EutranMeasurementMapping::IeValue2ActualHysteresis(reportConfig.hysteresis); //Fisxed value;
	m_curA3OffVal = EutranMeasurementMapping::IeValue2ActualA3Offset(reportConfig.a3Offset);
	m_curTTTVal = reportConfig.timeToTrigger;
	Simulator::Schedule (MilliSeconds(200), &LteEnbRrc::InitializeDSONParams, this);
	return measId;
}

void
LteEnbRrc::InitializeDSONParams(){
	m_dSONSapUser->SetUpInitialHandoverParams(m_curHysVal, m_curA3OffVal, m_curTTTVal, m_cellsToAddModMap);
}

void
LteEnbRrc::DoTriggerHandover (uint16_t rnti, uint16_t targetCellId)
{
	NS_LOG_LOGIC ("LteEnbRrc::DoTriggerHandover RNTI " << rnti << " targetCellId " << targetCellId << " sourceCellId " << m_cellId);

	bool isHandoverAllowed = true;

	if (m_anrSapProvider != 0)
	{
		// ensure that proper neighbor relationship exists between source and target cells
		bool noHo = m_anrSapProvider->GetNoHo (targetCellId);
		bool noX2 = m_anrSapProvider->GetNoX2 (targetCellId);
		NS_LOG_INFO (this << " cellId=" << m_cellId
...
{
	NS_LOG_LOGIC ("LteEnbRrc::DoSetPdschConfigDedicated RNTI " << rnti << " cellId " << m_cellId);
	Ptr<UeManager> ueManager = GetUeManager (rnti);
...
LteEnbRrc::AddUe (UeManager::State state)
{
	...
	m_ueMap.insert (std::pair<uint16_t, Ptr<UeManager> > (rnti, ueManager)); //create new mapping of {rnti,ueManager object}
	ueManager->Initialize ();
	NS_LOG_LOGIC("LteEnbRrc::AddUe " << " New UE RNTI " << rnti << " cellId " << m_cellId << " srs CI " << ueManager->GetSrsConfigurationIndex ());
...
{
	NS_LOG_DEBUG ("LteEnbRrc::RemoveUe RNTI " << (uint32_t) rnti << " cellId " << m_cellId);
	/*
	 * Fix: Remove Ue with unknown rnti (may be 2 function is called with same rnti)
	 * 2015/30/10
	 */
	std::map <uint16_t, Ptr<UeManager> >::iterator it = m_ueMap.find (rnti);
	...
	{
		NS_LOG_UNCOND("LteEnbRrc::RemoveUe request to remove UE info with unknown cellId " << m_cellId << " rnti " << rnti);
	}
...
LteEnbRrc::GetRlcType (EpsBearer bearer)
{
	...
	case RLC_AM_ALWAYS:
		NS_LOG_DEBUG("LteEnbRRC: RLC_UM_ALWAYS");
	...
	case PER_BASED:
		...
		{
			NS_LOG_DEBUG("PER_BASED: RLC_UM_ALWAYS");
			return LteRlcUm::GetTypeId ();
		}
		else
		{
			NS_LOG_DEBUG("PER_BASED: RLC_AM_ALWAYS");
			return LteRlcAm::GetTypeId ();
		}
		break;
...
LteEnbRrc::RemoveSrsConfigurationIndex (uint16_t srcCi)
{
	...
	NS_LOG_LOGIC("LteEnbRrc::RemoveSrsConfigurationIndex cellId " << m_cellId << " srcCi " << srcCi);
	std::set<uint16_t>::iterator it = m_ueSrsConfigurationIndexSet.find (srcCi);
...
/*********************************************************Developed by THANG*********************************************************/
/*
 * simple MDT functions
 */

void
LteEnbRrc::SetInitialCIOValues(std::map<uint16_t,int8_t> cioVals){
	//Call from LteHelper;
	m_cellsToAddModMap = cioVals;
}

uint8_t
LteEnbRrc::AddUeMeasReportConfigForMDT(){
	NS_LOG_FUNCTION (this);
	uint8_t measId = 0;
	if(m_enableMDT == true){
		LteRrcSap::ReportConfigEutra mdtMeasConfig;
		mdtMeasConfig.triggerQuantity = LteRrcSap::ReportConfigEutra::RSRP;
		mdtMeasConfig.triggerType = LteRrcSap::ReportConfigEutra::PERIODICAL;
		switch (m_mdtInterval)
		{
		case 120:
			mdtMeasConfig.reportInterval = LteRrcSap::ReportConfigEutra::MS120;
			break;

		case 240:
			mdtMeasConfig.reportInterval = LteRrcSap::ReportConfigEutra::MS240;
			break;

		case 480:
			mdtMeasConfig.reportInterval = LteRrcSap::ReportConfigEutra::MS480;
			break;

		case 640:
			mdtMeasConfig.reportInterval = LteRrcSap::ReportConfigEutra::MS640;
			break;

		case 1024:
			mdtMeasConfig.reportInterval = LteRrcSap::ReportConfigEutra::MS1024;
			break;

		default:
			NS_FATAL_ERROR ("Unsupported MDT ReportInterval " << (uint16_t) m_mdtInterval);
			break;
		}

		measId = AddUeMeasReportConfig (mdtMeasConfig);
		m_mdtMeasIDs.insert (measId);
		NS_ASSERT(measId > 0);
	}else{
		NS_LOG_LOGIC("LteEnbRrc::ConfigureCell MDT is not active at cellId " << m_cellId);
	}
	return measId;
}
/*
 ...
 */
void
LteEnbRrc::EnbRrcRecvDSONParamResp(SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList){
	m_s1SapProvider->EnbRrcSendDSONParamRespEnbS1Ap(m_cellId,msgInfo,paramList);
}

void LteEnbRrc::EnbRrcRecvUpdateParamConfig(SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList){
	m_dSONSapUser->EnbRrcSendUpdateParamConfigDSON(msgInfo,paramList);
}

void
LteEnbRrc::EnbRrcRecvDSONParamReq(SONCommon::SONMsgInfo msgInfo){
	m_dSONSapUser->EnbRrcSendDSONParamReqDSON(msgInfo);
}

void
LteEnbRrc::EnbRrcSetMroParams(LteEnbDSONSapProvider::MroParameters params){
	//Update control parameters
	m_curA3OffVal = params.a3Offset;
	m_curTTTVal = params.ttt;
	m_curHysVal = params.hyst;
	m_cellsToAddModMap = params.cioList;
	SaveHystA3OffsetTTTToLog(m_curHysVal, m_curA3OffVal,m_curTTTVal);
	SaveCIOValuesToLog();

	NS_ASSERT(params.cioList.size() > 0);
	(((m_ueMeasConfig.measObjectToAddModList).front()).measObjectEutra.cellsToAddModList).clear();//Clear old CIO values
	uint8_t cellIndex = 1;
	for(std::map<uint16_t, int8_t>::iterator itCIO = params.cioList.begin(); itCIO != params.cioList.end(); ++itCIO)
	{
		LteRrcSap::CellsToAddMod cellsToAddMod;
		cellsToAddMod.physCellId = itCIO->first;
		cellsToAddMod.cellIndividualOffset = itCIO->second;
		cellsToAddMod.cellIndex = cellIndex;
		cellIndex++;
		(((m_ueMeasConfig.measObjectToAddModList).front()).measObjectEutra.cellsToAddModList).push_back(cellsToAddMod);
	}

	int8_t a3OffsetIeValue = EutranMeasurementMapping::ActualA3Offset2IeValue (params.a3Offset);
	LteRrcSap::ReportConfigEutra reportConfig;
	reportConfig.eventId = LteRrcSap::ReportConfigEutra::EVENT_A3;
	reportConfig.a3Offset = a3OffsetIeValue;
	reportConfig.hysteresis = EutranMeasurementMapping::ActualHysteresis2IeValue(params.hyst);
	reportConfig.timeToTrigger = params.ttt; //ms
	...
	bool measConfigIdfound = false;
	std::list<LteRrcSap::ReportConfigToAddMod>::iterator it;
	for (it = m_ueMeasConfig.reportConfigToAddModList.begin();
			(it != m_ueMeasConfig.reportConfigToAddModList.end()) && (!measConfigIdfound);	it++)
	{
		if(it->reportConfigId == m_curMeasConfigIdHO)
		{//Look up ReportConfigEutra
			it->reportConfigEutra = reportConfig;//then modify it
			measConfigIdfound = true;
		}
	}

	if(measConfigIdfound == false){
		NS_FATAL_ERROR("LteEnbRrc::EnbRrcSetMroParams Can not find reportConfigId");
	}

	LteRrcSap::RrcConnectionReconfiguration rrcConRecfMsg;
	rrcConRecfMsg.haveMobilityControlInfo = false;
	rrcConRecfMsg.haveRadioResourceConfigDedicated = false;
	rrcConRecfMsg.haveMeasConfig = true;
	rrcConRecfMsg.measConfig = m_ueMeasConfig;
	//Send RrcConnectionReconfiguration to all users
	if(m_ueMap.empty() == true){
		NS_LOG_DEBUG("LteEnbRrc::EnbRrcSetMroParams m_ueMap is empty at cellId " << m_cellId );
	}else{
		for (std::map<uint16_t, Ptr<UeManager> >::iterator ueIt = m_ueMap.begin(); ueIt != m_ueMap.end(); ueIt++){
			ueIt->second->ModifyReportConfigRequest(rrcConRecfMsg);
		}
	}
}
...
void
LteEnbRrc::RrcSetMroParams(bool updateReq, LteEnbDSONSapProvider::MroParameters params){
	double time = Simulator::Now ().GetNanoSeconds () / (double) 1e9;
	NS_LOG_LOGIC("LteEnbRrc::EnbRrcSetMroParams at time " << time << " cellId " << m_cellId);
	//Reset global counters
	//	m_NumTooLateHO = 0;
	//	m_NumHOs = 0;
	//	m_NumWrongCellHO = 0;
	//	m_NumTooEarlyHO = 0; //reset status parameters
	//	m_NumPingpongHO = 0;
	//			m_aggNumHO = 0;
	//			m_aggSq = 0;
	//			m_aggSum = 0;
	m_hoIssueInfo.clear(); //Clear the database for next algorithm
	...
	if(updateReq){
		...
		for (it = m_ueMeasConfig.reportConfigToAddModList.begin();
				(it != m_ueMeasConfig.reportConfigToAddModList.end()) && (!measConfigIdfound);	it++){
			if(it->reportConfigId == m_curMeasConfigIdHO){//Look up ReportConfigEutra
				it->reportConfigEutra = reportConfig;//then modify it
				measConfigIdfound = true;
			}
		}
...
void
LteEnbRrc::RrcSetA3A4Params(LteEnbDSONSapProvider::MroParameters params, uint8_t A4thresh){
	//Update control parameters
	m_curA3OffVal = params.a3Offset;
	m_curTTTVal = params.ttt;
	m_curHysVal = params.hyst;
	m_cellsToAddModMap = params.cioList;
	NS_LOG_UNCOND("LteEnbRrc::RrcSetA3A4Params: A4thresh " << (uint32_t)A4thresh << " A3Offset " << (uint32_t)m_curA3OffVal
			<< " TTT " << (uint32_t)m_curTTTVal << " Hys " << (uint32_t)m_curHysVal);
	...
	reportConfigA3.hysteresis = EutranMeasurementMapping::ActualHysteresis2IeValue(params.hyst);
	reportConfigA3.timeToTrigger = params.ttt; //ms
	...
	for (it = m_ueMeasConfig.reportConfigToAddModList.begin();
			(it != m_ueMeasConfig.reportConfigToAddModList.end()) && (!measConfigIdfound);	it++){
		if(it->reportConfigId == m_curMeasConfigIdHO){//Look up ReportConfigEutra for handover
			it->reportConfigEutra = reportConfigA3;//then modify it
			measConfigIdfound = true;
		}
	}

	//Update A4 Parameters
	...
	reportConfigA4.threshold1.range = A4thresh; // intentionally very low threshold
	...
	measConfigIdfound = false;
	//	std::list<LteRrcSap::ReportConfigToAddMod>::iterator it;
	for (it = m_ueMeasConfig.reportConfigToAddModList.begin();
			(it != m_ueMeasConfig.reportConfigToAddModList.end()) && (!measConfigIdfound);	it++){
		if(it->reportConfigId == m_curMeasConfigIdAnr){//Look up ReportConfigEutra for handover
			it->reportConfigEutra = reportConfigA4;//then modify it
			measConfigIdfound = true;
		}
	}
...
/*
 * Send Handover Update to CSON via S1-AP
 */
void
LteEnbRrc::EnbRrcRcvPMReportNotify(SONCommon::SONMsgInfo msgInfo){
	switch (msgInfo) {
	case SONCommon::MRO:
	{
		SONCommon::HandoverStatisticReport hoStatistic;
		//			hoStatistic.numHOs = m_NumHOs;
		//			hoStatistic.numTooEarlyHos = m_NumTooEarlyHO;
		//			hoStatistic.numTooLateHos = m_NumTooLateHO;
		//			hoStatistic.numWrongCellHos = m_NumWrongCellHO;
		//			hoStatistic.numPingpongHos = m_NumPingpongHO;

		std::map<uint16_t,LteEnbDSONSapUser::HandoverIssueInfo>::iterator it;
		for(it = m_hoIssueInfo.begin(); it != m_hoIssueInfo.end(); it++){
			SONCommon::CellStatistic cellStatistic;
			cellStatistic.m_numEHO = it->second.m_numEHO;
			cellStatistic.m_numLHO = it->second.m_numLHO;
			cellStatistic.m_numWHO = it->second.m_numWHO;
			cellStatistic.m_numPP = it->second.m_numPingpong;
			hoStatistic.hoStatistic[it->first] = cellStatistic; //Gather neighbor cell information
		}
		SONCommon::PerformanceReport pmReport;
		pmReport.hoReport = hoStatistic;
	}
	break;
	default:
		NS_FATAL_ERROR("LteEnbRrc::EnbRrcRcvPMReportNotify Info is not implemented yet");
		break;
	}
...
void
LteEnbRrc::SetLteEnbDSONSapUser(LteEnbDSONSapUser* dSONUser){
	m_dSONSapUser = dSONUser;
}

LteEnbDSONSapProvider*
LteEnbRrc::GetLteEnbDSONSapProvider(){
	return m_dSONSapProvider;
}

void
LteEnbRrc::SetHandoverCIOParams(void){
	//Update new CIO values for cells
	std::list<LteRrcSap::CellsToAddMod>::iterator cellModIt = (((m_ueMeasConfig.measObjectToAddModList).front()).measObjectEutra.cellsToAddModList).begin();
	if((((m_ueMeasConfig.measObjectToAddModList).front()).measObjectEutra.cellsToAddModList).size() != m_cellsToAddModMap.size()){
		NS_FATAL_ERROR("Error cellsToAddModList size");
	}

	for(uint8_t cellIndex = 1; cellIndex <= m_cellsToAddModMap.size(); cellIndex++){
		cellModIt->cellIndividualOffset = m_cellsToAddModMap[cellIndex];
		cellModIt++;
	}
	//	//According to 5.5.2.7 TS 36.331
	//	//Create new report config EUTRA object with new CIO values
	...
	if(m_ueMap.empty()){
		NS_LOG_LOGIC("There is no Ue to send RrcConnectionReconfiguration request");
	}else{
		for (std::map<uint16_t, Ptr<UeManager> >::iterator ueIt = m_ueMap.begin(); ueIt != m_ueMap.end(); ueIt++){
			rrcConRecfMsg.rrcTransactionIdentifier = ueIt->second->GetNewRrcTransactionIdentifier();
			//			NS_LOG_DEBUG("UeManager::SetHandoverCIOParams RNTI " << ueIt->first << " cellId " << m_cellId);
			ueIt->second->ModifyReportConfigRequest(rrcConRecfMsg);
		}
	}
...
void
LteEnbRrc::SaveCIOValuesToLog(){
	std::ostringstream converter; //convert integer to string
	converter << m_cellId;
	std::string fileName = "CIOTracingCellId"+converter.str()+".txt";
	std::ofstream outFile;
	if (m_cioValuesFirstWrite == true){
		outFile.open (fileName.c_str());
		if (!outFile.is_open ()){
			NS_LOG_ERROR ("Can't open file " << fileName.c_str());
			return;
		}
		m_cioValuesFirstWrite = false;
		outFile << "time\tsCellId\tCIOlist";
		outFile << std::endl;
	}else{
	...
	outFile << time << "\t";
	outFile << m_cellId << "\t";
	for(std::map<uint16_t,int8_t>::iterator cioIt = m_cellsToAddModMap.begin(); cioIt != m_cellsToAddModMap.end(); cioIt++){
		outFile << "(" << cioIt->first << "," << m_cioResol*cioIt->second << ") ";
	}
	outFile << std::endl;
	outFile.close ();
}
...
void
LteEnbRrc::SetupCellToAddModList(){
	//    (((m_ueMeasConfig.measObjectToAddModList).front()).measObjectEutra.cellsToAddModList).clear(); //Firstly clear the old CIO values
	NS_ASSERT(m_cellsToAddModMap.size() > 0);
	LteRrcSap::CellsToAddMod cellsToAddMod;
	cellsToAddMod.cellIndividualOffset = 0; // Default CIO value
	cellsToAddMod.cellIndex = 0; // Default CIO value
	cellsToAddMod.physCellId = 0;

	for(std::map<uint16_t,int8_t>::iterator mapIt = m_cellsToAddModMap.begin(); mapIt != m_cellsToAddModMap.end(); mapIt ++){
		cellsToAddMod.cellIndividualOffset = mapIt->second; // Set CIO values
		cellsToAddMod.physCellId = mapIt->first; //Manually Set CellId values
		cellsToAddMod.cellIndex += 1;
		(((m_ueMeasConfig.measObjectToAddModList).front()).measObjectEutra.cellsToAddModList).push_back(cellsToAddMod); //Secondly add new CIO values
	}

	NS_LOG_UNCOND("CellId: " << m_cellId);
	for(std::map<uint16_t,int8_t>::iterator mapIt = m_cellsToAddModMap.begin(); mapIt != m_cellsToAddModMap.end(); mapIt ++)
	{
		NS_LOG_UNCOND("    nCellId: "<< mapIt->first << " CIO: " << (double)mapIt->second);
	}
}

void
LteEnbRrc::RemoveHandoverdUeContextForEHO(LteEnbRrc::HandOverUeContext hoUeContext){
	//New update will be used only pop-front, because this buffer likes a FIFO
	//Check valid time, UeContext being released just after running algorithm is neglected
	if(Simulator::Now ().GetSeconds() - m_lastUpdateTime.GetSeconds() >= m_UeCxtTimerForEHO.GetSeconds() ){//modifying For continuous update
		//		if((hoUeContext.m_rnti == m_UeContextForEHO.front().m_rnti) &&
		//						(hoUeContext.m_cellId == m_UeContextForEHO.front().m_cellId)){//just for debug
		NS_ASSERT((hoUeContext.m_rnti == m_UeContextForEHO.front().m_rnti) && (hoUeContext.m_cellId == m_UeContextForEHO.front().m_cellId));
		LteEnbDSONSapUser::HandoverIssueInfo newTracing;
		newTracing.m_ehoDetected = false;
		newTracing.m_whoDetected = false;
		newTracing.m_lhoDetected = false;
		newTracing.m_ppDetected = false;
		newTracing.m_numEHO = 0;
		newTracing.m_numWHO = 0;
		newTracing.m_numLHO = 0;
		newTracing.m_numPingpong = 0;
		/*
		 * Update Error Wrong Cell Handover Statistic 2015/11/14
		 * Discriminate Wrong Cell and Too Early Handover
		 */
		if(m_UeContextForEHO.front().m_ehoDetected == true){
			//				m_NumTooEarlyHO++;
			newTracing.m_ehoDetected = true;
			newTracing.m_numEHO = 1;
		}

		if(m_UeContextForEHO.front().m_whoDetected == true){
			//				m_NumWrongCellHO++;
			newTracing.m_whoDetected = true;
			newTracing.m_numWHO = 1;
		}

		if(m_UeContextForEHO.front().m_ppDetected == true){
			//				m_NumPingpongHO++;
			newTracing.m_ppDetected = true;
			newTracing.m_numPingpong = 1;
		}

		//Update Handover Database
		std::map<uint16_t,LteEnbDSONSapUser::HandoverIssueInfo>::iterator tracingIt = m_hoIssueInfo.find(hoUeContext.m_cellId); //Look up the target cell database
		if(tracingIt == m_hoIssueInfo.end()){//New entry
			m_hoIssueInfo[hoUeContext.m_cellId] = newTracing;
		}else{//entry existed
			if(newTracing.m_ehoDetected == true){
				(tracingIt->second).m_numEHO++;
			}
			if(newTracing.m_whoDetected == true){
				(tracingIt->second).m_numWHO++;
			}
			if(newTracing.m_ppDetected == true){
				(tracingIt->second).m_numPingpong++;
			}
		}
		//Trace at pThread (Main)
		if(newTracing.m_ehoDetected == true)
		{
			SONCommon::MROProblemInfo proInfo;
			proInfo.cellId = hoUeContext.m_cellId; //neighbor cell Id
			proInfo.imsi = hoUeContext.m_imsi;
			proInfo.measData = m_UeContextForEHO.front().m_rlfMeasData;
			proInfo.problem = SONCommon::TOO_EARLY_HANDOVER;
			m_recvMroStatData(m_cellId,proInfo);
		}
		if(newTracing.m_whoDetected == true)
		{
			...
			proInfo.imsi = hoUeContext.m_imsi;
			proInfo.measData = m_UeContextForEHO.front().m_rlfMeasData;
			proInfo.problem = SONCommon::WRONG_CELL_HANDOVER;
			m_recvMroStatData(m_cellId,proInfo);
		}
		if(newTracing.m_ppDetected == true)
		{
			...
			proInfo.cellId = hoUeContext.m_cellId; //neighbor cell Id
			proInfo.imsi = hoUeContext.m_imsi;
			proInfo.problem = SONCommon::PING_PONG_HANDOVER;
			m_recvMroStatData(m_cellId,proInfo);
		}

		if((newTracing.m_ehoDetected == false) && (newTracing.m_whoDetected == false) && (newTracing.m_ppDetected == false))
		{
			//Update only when there is no HO error to reduce system load at Pthread
			SONCommon::MROProblemInfo proInfo;
			proInfo.cellId = hoUeContext.m_cellId; //neighbor cell Id
			proInfo.imsi = hoUeContext.m_imsi;
			proInfo.problem = SONCommon::NORMAL_HANDOVER;
			m_recvMroStatData(m_cellId,proInfo);
		}

		//		m_NumHOs++; //synchronize #handover variable with #late, #early, and #wrong
	}

	m_UeContextForEHO.pop_front();//Remove top of buffer
}

void
LteEnbRrc::RemoveHandoverdUeContextForWHO(LteEnbRrc::HandOverUeContext hoUeContext){
	if((hoUeContext.m_rnti == m_UeContextForWHO.front().m_rnti) &&
			(hoUeContext.m_cellId == m_UeContextForWHO.front().m_cellId)){
		NS_LOG_LOGIC("LteEnbRrc::RemoveHandoverdUeContextForWHO RNTI " << hoUeContext.m_rnti << " cellID " << hoUeContext.m_cellId);
	}else{
		NS_FATAL_ERROR("LteEnbRrc::RemoveHandoverdUeContextForWHO No Handover Ue Context found targetcellId " << hoUeContext.m_cellId << " RNTI " << hoUeContext.m_rnti);
	}

	m_UeContextForWHO.pop_front(); //First push back, Then pop front
}

void
LteEnbRrc::SetHandoverControlParams(double a3Offset, uint16_t timeToTrigger){

	LteRrcSap::RrcConnectionReconfiguration rrcConRecfMsg =
			BuildRrcConnectionReconfigurationForHandoverParameters(a3Offset,timeToTrigger);
	//Send RrcConnectionReconfiguration to specific users
	if(m_ueMap.empty()){
		NS_LOG_LOGIC("There is no Ue to send RrcConnectionReconfiguration request");
	}else{
		for (std::map<uint16_t, Ptr<UeManager> >::iterator it = m_ueMap.begin();
				it != m_ueMap.end();
				it++){
			rrcConRecfMsg.rrcTransactionIdentifier = it->second->GetNewRrcTransactionIdentifier();
			NS_LOG_LOGIC("UeManager::ModifyReportConfigRequest cellId " << m_cellId << " RNTI " << it->first << " IMSI " << it->second->GetImsi());
			it->second->ModifyReportConfigRequest(rrcConRecfMsg);
		}
	}
}

void
LteEnbRrc::SetANRControlParams(uint8_t rsrpThreshold){
	LteRrcSap::RrcConnectionReconfiguration rrcConRecfMsg =
			BuildRrcConnectionReconfigurationForANR(rsrpThreshold);
	//Send RrcConnectionReconfiguration to specific users
	if(m_ueMap.empty()){
		NS_LOG_LOGIC("There is no Ue to send RrcConnectionReconfiguration request");
	}else{
...
void
LteEnbRrc::SaveHystA3OffsetTTTToLog(double hyst, double a3ofsetVal, uint16_t tttVal){
	std::ostringstream converter; //convert integer to string
	converter << m_cellId;
	std::string fileName = "A3TTTCell"+converter.str()+".txt";
	std::ofstream outFile;
	if (m_A3TTTFirstWrite == true){
		outFile.open (fileName.c_str());
		if (!outFile.is_open ()){
			NS_LOG_ERROR ("Can't open file " << fileName.c_str());
			return;
		}
		m_A3TTTFirstWrite = false;
		outFile << "time\tsCellId\tHysteresis\tA3Offset\tTTT";
		outFile << std::endl;
	}else{
		outFile.open (fileName.c_str(),  std::ios_base::app);
		if (!outFile.is_open ()){
	...
	outFile << hyst << "\t";
	outFile << a3ofsetVal << "\t";
	outFile << tttVal << std::endl;
	outFile.close ();
}

LteRrcSap::RrcConnectionReconfiguration
LteEnbRrc::BuildRrcConnectionReconfigurationWithoutDedicatedConfig(){
	LteRrcSap::RrcConnectionReconfiguration msg;
	msg.haveRadioResourceConfigDedicated = false;
	msg.haveMobilityControlInfo = false;
	msg.haveMeasConfig = true;
	msg.measConfig = m_ueMeasConfig;
	return msg;
}

LteRrcSap::RrcConnectionReconfiguration
LteEnbRrc::BuildRrcConnectionReconfigurationForHandoverParameters(double a3Offset, uint16_t timeToTrigger){
	int8_t a3OffsetIeValue = EutranMeasurementMapping::ActualA3Offset2IeValue (a3Offset);
	LteRrcSap::ReportConfigEutra reportConfig;
	reportConfig.eventId = LteRrcSap::ReportConfigEutra::EVENT_A3;
	reportConfig.a3Offset = a3OffsetIeValue;
	reportConfig.hysteresis = EutranMeasurementMapping::ActualHysteresis2IeValue(m_curHysVal);
	reportConfig.timeToTrigger = timeToTrigger; //ms
	reportConfig.reportOnLeave = false;
	reportConfig.triggerQuantity = LteRrcSap::ReportConfigEutra::RSRP;
	reportConfig.reportInterval = LteRrcSap::ReportConfigEutra::MS1024;

	//modify measConfig with new reportConfigEUTRA
	bool measConfigIdfound = false;
	for (std::list<LteRrcSap::ReportConfigToAddMod>::iterator it = m_ueMeasConfig.reportConfigToAddModList.begin();
			(it != m_ueMeasConfig.reportConfigToAddModList.end()) && (!measConfigIdfound);	it++){
		if(it->reportConfigId == m_curMeasConfigIdHO){
	...
	//	if(!measConfigIdfound){
	//		NS_FATAL_ERROR("No Handover Algorithm ->No MeasConfigId with Id " << (uint32_t)m_curMeasConfigIdHO);
	//	}

	//add reportConfig object to RrcConnectionReconfiguration message
	LteRrcSap::RrcConnectionReconfiguration rrcConRecfMsg;
	rrcConRecfMsg.haveMobilityControlInfo = false;
	rrcConRecfMsg.haveRadioResourceConfigDedicated = false;
	rrcConRecfMsg.haveMeasConfig = true;
	rrcConRecfMsg.measConfig = m_ueMeasConfig;
	return rrcConRecfMsg;
}

LteRrcSap::RrcConnectionReconfiguration
LteEnbRrc::BuildRrcConnectionReconfigurationForANR(uint8_t rsrpThreshold){
	LteRrcSap::ReportConfigEutra reportConfig;
	...
	reportConfig.threshold1.range = rsrpThreshold;
	reportConfig.triggerQuantity = LteRrcSap::ReportConfigEutra::RSRP;
	...
	for (std::list<LteRrcSap::ReportConfigToAddMod>::iterator it = m_ueMeasConfig.reportConfigToAddModList.begin();
			(it != m_ueMeasConfig.reportConfigToAddModList.end()) && (!measConfigIdfound);	it++){
		if(it->reportConfigId == m_curMeasConfigIdAnr){
			it->reportConfigEutra = reportConfig;
			measConfigIdfound = true;
		}
	}
...
void
LteEnbRrc::DoRecvRlfIndicationMessage(EpcX2SapUser::RlfIndicationMessageParams params){
	if(params.forwarded)
	{
		bool ueContextFound = false;
		for(std::list<HandOverUeContext>::iterator it = m_UeContextForEHO.begin();
				(it != m_UeContextForEHO.end()) && (!ueContextFound); it++){
			if((it->m_rnti == params.failureCRNTI) && (it->m_cellId == params.reportCellId))
			{
				ueContextFound = true;
				it->m_whoDetected = true;
				it->m_rlfMeasData = params.rlfMeasData;
			}
		}
	}
	else
	{
		bool founded = false;
		if (m_UeContextForWHO.empty() == false) //Consider last update time??
		{
			EpcX2SapProvider::RlfIndicationMessageParams rlfMsgParams;
			for(std::list<LteEnbRrc::HandOverUeContext>::iterator it = m_UeContextForWHO.begin();
					(it != m_UeContextForWHO.end()) && (!founded); it++)
			{
				if(((it->m_rnti) == (params.failureCRNTI)) && ((it->m_cellId) != (params.reportCellId)))
				{
					rlfMsgParams.failureCRNTI = params.failureCRNTI;
					rlfMsgParams.failureIMSI = params.failureIMSI;
					rlfMsgParams.failureCellId = params.failureCellId;
					rlfMsgParams.reportCellId = m_cellId;
					rlfMsgParams.forwarded = true; //Detect Wrong Cell Handover
					rlfMsgParams.sourceCellId = it->m_cellId;
					rlfMsgParams.rlfMeasData = params.rlfMeasData;
					founded = true;
				}
			}
			if(founded == true)
			...
			{
//				if (m_uePacketBuffer.find(params.failureCRNTI) != m_uePacketBuffer.end())
//				{
//					if(m_uePacketBuffer[params.failureCRNTI].size() > 0)
//					{
//						Ptr<Packet> bufferedPacket = m_uePacketBuffer[params.failureCRNTI].front();
//						m_uePacketBuffer[params.failureCRNTI].pop();
//						EpsBearerTag packetTag;
//						bool tagFound = bufferedPacket->RemovePacketTag (packetTag);
//						NS_ASSERT_MSG(tagFound && (params.failureCRNTI == packetTag.GetRnti()),"no EpsBearerTag found in packet to be sent");
//						//						uint8_t drbid = packetTag.GetBid();//Bid2Drbid (bid);
//						EpcX2Sap::UeDataParams x2params;
//						x2params.sourceCellId = m_cellId;
//						x2params.targetCellId = params.reportCellId;
//						x2params.gtpTeid = params.newTeId; // in NS3: X2 GTP-U TeId = S1 GTP-U TeId ~ S5 GTP-U TeId
//						x2params.ueData = bufferedPacket;
//						m_x2SapProvider->SendUeData (x2params);
//					}
//				}
				...
				newTracing.m_ehoDetected=false;
				newTracing.m_whoDetected=false;
				newTracing.m_lhoDetected = true;
				newTracing.m_numLHO = 1;
				newTracing.m_numEHO=0;
				newTracing.m_numWHO=0;
				std::map<uint16_t,LteEnbDSONSapUser::HandoverIssueInfo>::iterator traceIt = m_hoIssueInfo.find(params.reportCellId);
				if(traceIt == m_hoIssueInfo.end())
				{//new entry
					m_hoIssueInfo[params.reportCellId] = newTracing;
				}
				else
				{
					(traceIt->second).m_lhoDetected = true;
					(traceIt->second).m_numLHO++;
				}
				//Send to trace sink at main
				SONCommon::MROProblemInfo proInfo;
				proInfo.cellId = params.reportCellId; //neighbor cell Id params.failureIMSI
				proInfo.imsi = params.failureIMSI;
				proInfo.problem = SONCommon::TOO_LATE_HANDOVER;
				proInfo.measData = params.rlfMeasData;
				m_recvMroStatData(m_cellId,proInfo);
			}
		}
		else
		{
			NS_LOG_LOGIC("LteEnbRrc::DoRecvRlfIndicationMessage m_UeContextForWHO is empty");
		}
	}
}
...
UeManager::RecvRrcConnectionReestablishmentComplete (LteRrcSap::RrcConnectionReestablishmentComplete msg)
{
	...
	case CONNECTION_REESTABLISHMENT:
	{
		m_connectionReestablishmentTimeout.Cancel();
		StartDataRadioBearers ();
		SwitchToState (CONNECTED_NORMALLY);
		LteRrcSap::RrcConnectionReconfiguration rrcConRecfMsg =
				m_rrc->BuildRrcConnectionReconfigurationForHandoverParameters(m_rrc->m_curA3OffVal, m_rrc->m_curTTTVal); //Request Ue Update New Params
		rrcConRecfMsg.rrcTransactionIdentifier = GetNewRrcTransactionIdentifier();
		ModifyReportConfigRequest(rrcConRecfMsg);
	}
	break;
...
void
UeManager::ModifyReportConfigRequest(LteRrcSap::RrcConnectionReconfiguration msg){
	...
	default:
		NS_LOG_LOGIC("UeManager::ModifyReportConfigRequest not support " << ToString(m_state));
		break;
	}
}
void
LteEnbRrc::SetUeCxtStoreTimer(Time time){
	NS_LOG_DEBUG("LteEnbRrc::SetUeCxtStoreTimer cellId" << m_cellId << "Set UeCxtTimer " << time.GetSeconds());
	m_UeCxtTimerForEHO = time;
	m_UeCxtTimerForWHO = time;
}
Time
LteEnbRrc::GetUeCxtStoreTimer() const{
	return m_UeCxtTimerForEHO;
}
