 ...
 */
double
EutranMeasurementMapping::IeValue2ActualCIO(int8_t cioValue){
	if ((cioValue < -24) || (cioValue > 24))
	{
	    NS_FATAL_ERROR ("The value " << (int16_t) cioValue
	                   << " is out of the allowed range (-24..24)"
	                   << " for Q-OffsetRange IE value");
	}

	double actual = static_cast<double> (cioValue);
	NS_ASSERT (actual >= -24.0);
	NS_ASSERT (actual <= 24.0);
	return actual;
}

double
EutranMeasurementMapping::IeValue2ActualCIONew(double cioValue){
//	if ((cioValue < -24) || (cioValue > 24))
//	{
//	    NS_FATAL_ERROR ("The value " << (int16_t) cioValue
//	                   << " is out of the allowed range (-24..24)"
 ...
 */
int8_t
EutranMeasurementMapping::ActualCIOIeValue(double cioValue)
{
	if ((cioValue < -24.0) || (cioValue > 24.0))
	{
		NS_FATAL_ERROR ("The value " << cioValue
				<< " is out of the allowed range (-24..24) dB"
				<< " for Q- OffsetRange");
	}

	int8_t ieValue = lround (cioValue * 1.0);
	NS_ASSERT (ieValue >= -24);
	NS_ASSERT (ieValue <= 24);
	return ieValue;
}

double
EutranMeasurementMapping::ActualCIOIeValueNew(double cioValue)
{
//	if ((cioValue < -24.0) || (cioValue > 24.0))
//	{
//		NS_FATAL_ERROR ("The value " << cioValue
