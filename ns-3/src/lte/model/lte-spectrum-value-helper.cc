...
LteSpectrumValueHelper::CreateTxPowerSpectralDensity (uint16_t earfcn, uint8_t txBandwidthConfiguration, double powerTx, std::vector <int> activeRbs)
{
  ...
  if(activeRbs.size() > 0){//THANG debug
	  NS_LOG_LOGIC ("LteSpectrumValueHelper::CreateTxPowerSpectralDensity for earfcn " << earfcn <<
			  " bw " <<  (uint16_t) txBandwidthConfiguration << " txpower " << powerTx
			  << " #activeRbs " << activeRbs.size() << " startRB " << activeRbs.at(0) << " txpd " << txPowerDensity);
  }else{
	  NS_LOG_LOGIC ("LteSpectrumValueHelper::CreateTxPowerSpectralDensity for earfcn " << earfcn <<
			  " bw " <<  (uint16_t) txBandwidthConfiguration << " txpower " << powerTx << " #activeRbs " << activeRbs.size() << " txpd " << txPowerDensity);
  }
