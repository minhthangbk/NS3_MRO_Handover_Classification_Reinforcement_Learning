...
RrFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  NS_LOG_LOGIC("RBG Size: " << rbgSize << " Number of RBGs: " << rbgNum);
  ...
  NS_LOG_INFO("Number of Allocated RBGs " << rbgAllocatedNum << " number of TBs " << nTbs);
  ...
  do
    {
      ...
      NS_LOG_INFO("RNTI: " << (uint32_t)((*it).m_rnti) << " nPRBs: " << (uint32_t)(rbgPerTb * rbgSize) <<  " MCS: " << (uint32_t)(newDci.m_mcs.at (0)));
      ...
      NS_LOG_INFO(this << "rbgAllocatedNum: " << (uint32_t)rbgAllocatedNum << " rbgAllocated: " << (uint32_t)rbgAllocated);
...
RrFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
  ...
  m_rachAllocationMap.clear ();// remove RACH allocation
  m_rachAllocationMap.resize (m_cschedCellConfig.m_ulBandwidth, 0);
  ...
  // Divide the remaining resources equally among the active users starting from the subsequent one which were served last scheduling trigger
  ...
  do
    {
      ...
      //THANG
      NS_LOG_LOGIC ("RrFfMacScheduler::DoSchedUlTriggerReq UL Allocation - UE " << (*it).first << " startPRB " <<
    		  (uint32_t)uldci.m_rbStart << " nPRB " << (uint32_t)uldci.m_rbLen << " CQI " <<
    		  cqi << " MCS " << (uint32_t)uldci.m_mcs << " TBsize " << uldci.m_tbSize << " harqId " << (uint16_t)harqId);
