...
#include "ns3/lte-rrc-sap.h" //THANG
  ...
  for (int j = 0; j < sz; j++)
    {
      ...
      erabItem.erabId = i.ReadNtohU16 ();//2
 
      erabItem.erabLevelQosParameters = EpsBearer ((EpsBearer::Qci) i.ReadNtohU16 ());//2
      erabItem.erabLevelQosParameters.gbrQosInfo.gbrDl = i.ReadNtohU64 ();//8
      erabItem.erabLevelQosParameters.gbrQosInfo.gbrUl = i.ReadNtohU64 ();//8
      erabItem.erabLevelQosParameters.gbrQosInfo.mbrDl = i.ReadNtohU64 ();//8
      erabItem.erabLevelQosParameters.gbrQosInfo.mbrUl = i.ReadNtohU64 ();//8
      erabItem.erabLevelQosParameters.arp.priorityLevel = i.ReadU8 ();//1
      erabItem.erabLevelQosParameters.arp.preemptionCapability = i.ReadU8 ();//1
      erabItem.erabLevelQosParameters.arp.preemptionVulnerability = i.ReadU8 ();//1
      erabItem.dlForwarding = i.ReadU8 ();//1
      erabItem.transportLayerAddress = Ipv4Address (i.ReadNtohU32 ());//4
      erabItem.gtpTeid = i.ReadNtohU32 ();//4

      m_erabsToBeSetupList.push_back (erabItem);
      m_headerLength += 48;//2+2+8+8+8+8+1+1+1+1+4+4
    }
...
EpcX2HandoverRequestAckHeader::EpcX2HandoverRequestAckHeader ()
  : m_numberOfIes (1 + 1 + 1 + 1 + 1), //THANG add '+ 1' for imsi
    m_headerLength (2 + 2 + 4 + 4 + 8), //THANG add '+ 8' for uint64_t
    m_oldEnbUeX2apId (0xfffa),
    m_newEnbUeX2apId (0xfffa),
    m_imsi(0xfffffffa) //THANG Add IMSI for pingpong Detection
{
}
...
EpcX2HandoverRequestAckHeader::Deserialize (Buffer::Iterator start)
{
  ...
  m_headerLength += 4 + 8; //THANG add '+ 8' for imsi
  m_numberOfIes += 2 + 1;  //THANG add '+ 1' for imsi
...
/*
 * THANG add Set/Get IMSI
 */
void
EpcX2HandoverRequestAckHeader::SetUeIMSI(uint64_t imsi){
	m_imsi = imsi;
}
uint64_t
EpcX2HandoverRequestAckHeader::GetUeIMSI()	const {
	return m_imsi;
}
/////////////////////////////////////////////////////////////////////
  ...
  for (int j = 0; j < sz; j++)
    {
      ...
      item.sourceCellId = i.ReadNtohU16 ();//2
      item.dlHardwareLoadIndicator = (EpcX2Sap::LoadIndicator) i.ReadU8 ();//1
      item.ulHardwareLoadIndicator = (EpcX2Sap::LoadIndicator) i.ReadU8 ();//1
      item.dlS1TnlLoadIndicator = (EpcX2Sap::LoadIndicator) i.ReadU8 ();//1
      item.ulS1TnlLoadIndicator = (EpcX2Sap::LoadIndicator) i.ReadU8 ();//1

      item.dlGbrPrbUsage = i.ReadNtohU16 ();//2
      item.ulGbrPrbUsage = i.ReadNtohU16 ();//2
      item.dlNonGbrPrbUsage = i.ReadNtohU16 ();//2
      item.ulNonGbrPrbUsage = i.ReadNtohU16 ();//2
      item.dlTotalPrbUsage = i.ReadNtohU16 ();//2
      item.ulTotalPrbUsage = i.ReadNtohU16 ();//2

      item.dlCompositeAvailableCapacity.cellCapacityClassValue = i.ReadNtohU16 ();//2
      item.dlCompositeAvailableCapacity.capacityValue = i.ReadNtohU16 ();//2
      item.ulCompositeAvailableCapacity.cellCapacityClassValue = i.ReadNtohU16 ();//2
      item.ulCompositeAvailableCapacity.capacityValue = i.ReadNtohU16 ();//2
...
/*****************************THANG******************************/
NS_OBJECT_ENSURE_REGISTERED (EpcX2RlfIndicationMessageHeader);

EpcX2RlfIndicationMessageHeader::EpcX2RlfIndicationMessageHeader():
				m_numberOfIes(1 + 1 + 1 + 1 + 1 + 1), //4 parameters: m_failureCellId, m_failureCRNTI, m_reportCellId, m_sourceCellId
				m_headerLength(2 + 8 + 2 + 2 + 2 + 8), //2 bytes + 2 bytes + 2 bytes + 2 bytes
				m_failureCRNTI(0xfffa), //Chose any value
				m_failureIMSI(0xfffa),
				m_failureCellId(0xfffa),
				m_reportCellId(0xfffa),
...
EpcX2RlfIndicationMessageHeader::~EpcX2RlfIndicationMessageHeader()
{
	m_numberOfIes = 0;
	m_headerLength = 0;
	m_failureCRNTI = 0xfffb; //Chose any value
	m_failureCellId = 0xfffb;
	m_reportCellId = 0xfffb;
	m_sourceCellId = 0xfffa;
	m_newTeId = 0;
	m_failureIMSI = 0;
}
...
TypeId
EpcX2RlfIndicationMessageHeader::GetTypeId(void){
	static TypeId tid = TypeId ("ns3::EpcX2RlfIndicationMessageHeader")
	    		.SetParent<Header> ()
	    		.SetGroupName("Lte")
	    		.AddConstructor<EpcX2RlfIndicationMessageHeader> ()
	    		;
	return tid;
}

TypeId
EpcX2RlfIndicationMessageHeader::GetInstanceTypeId (void) const
{
	return GetTypeId ();
}

uint32_t
EpcX2RlfIndicationMessageHeader::GetSerializedSize (void) const
{
	return m_headerLength;
}

void
EpcX2RlfIndicationMessageHeader::Serialize (Buffer::Iterator start) const
{
//	uint16_t realHL=0;
	Buffer::Iterator i = start;
	i.WriteHtonU16 (m_failureCRNTI);//2
	i.WriteHtonU64 (m_failureIMSI);//8
	i.WriteHtonU16 (m_failureCellId);//2
	i.WriteHtonU16 (m_reportCellId);//2
	i.WriteHtonU16 (m_sourceCellId);//2
	i.WriteHtonU64 (m_newTeId);//8

	std::list <LteRrcSap::MeasObjectEutra>::size_type sz = m_rlfMeasData.size ();
	i.WriteHtonU16 (sz);              // number of bearers

//	realHL = (2+8+2+2+2+8+2);

	for (std::list<LteRrcSap::MeasResultEutra>::const_iterator measIt=m_rlfMeasData.begin();measIt!=m_rlfMeasData.end();measIt++)
	{
//		i.WriteHtonU16 (measIt->haveCgiInfo);
//		i.WriteHtonU16 (measIt->haveRsrpResult);
//		i.WriteHtonU16 (measIt->haveRsrqResult);
		i.WriteHtonU16 (measIt->physCellId);
		i.WriteHtonU16 (measIt->rsrpResult);
		i.WriteHtonU16 (measIt->rsrqResult);
//		realHL += (2+2+2);
	}

//	NS_LOG_UNCOND("EpcX2RlfIndicationMessageHeader::Serialize FixedHL: " << m_headerLength << " realHL: " << realHL);
}

uint32_t
EpcX2RlfIndicationMessageHeader::Deserialize (Buffer::Iterator start)
{
	Buffer::Iterator i = start;

	m_failureCRNTI = i.ReadNtohU16 ();//2
	m_failureIMSI = i.ReadNtohU64();//8
	m_failureCellId = i.ReadNtohU16 ();//2
	m_reportCellId = i.ReadNtohU16 ();//2
	m_sourceCellId = i.ReadNtohU16();//2
	m_newTeId = i.ReadNtohU64();//8
	uint16_t measDataSize = i.ReadNtohU16();//2
	m_headerLength = (2+8+2+2+2+8+2);
	for(uint16_t j=1;j<=measDataSize;j++)
	{
		LteRrcSap::MeasResultEutra measEUTRA;
		measEUTRA.physCellId = i.ReadNtohU16();//2
		measEUTRA.rsrpResult = i.ReadNtohU16();//2
		measEUTRA.rsrqResult = i.ReadNtohU16();//2
		m_rlfMeasData.push_back(measEUTRA);
		m_headerLength += (2+2+2);
	}
...
void
EpcX2RlfIndicationMessageHeader::Print (std::ostream &os) const
{
	os << "m_failureCRNTI = " << m_failureCRNTI;
	os << " m_failureCellId = " << m_failureCellId;
	os << " m_reportCellId = " << m_reportCellId;
	os << " m_sourceCellId = " << m_sourceCellId;
}

uint16_t
EpcX2RlfIndicationMessageHeader::GetFailureCellId() const
{
	return m_failureCellId;
}

void
EpcX2RlfIndicationMessageHeader::SetFailureCellId(uint16_t failureCellId)
{
	m_failureCellId = failureCellId;
}

uint16_t
EpcX2RlfIndicationMessageHeader::GetFailureCRNTI() const
{
	return m_failureCRNTI;
}

void
EpcX2RlfIndicationMessageHeader::SetFailureCRNTI(uint16_t failureCRNTI)
{
	m_failureCRNTI = failureCRNTI;
}

uint64_t
EpcX2RlfIndicationMessageHeader::GetFailureIMSI() const
{
	return m_failureIMSI;
}

void
EpcX2RlfIndicationMessageHeader::SetFailureIMSI(uint64_t imsi)
{
	m_failureIMSI = imsi;
}

uint16_t
EpcX2RlfIndicationMessageHeader::GetReportCellId() const
{
	return m_reportCellId;
}

void
EpcX2RlfIndicationMessageHeader::SetReportCellId(uint16_t reportCRNTI)
{
	m_reportCellId = reportCRNTI;
}

uint16_t
EpcX2RlfIndicationMessageHeader::GetSourceCellId() const
{
	return m_sourceCellId;
}

void
EpcX2RlfIndicationMessageHeader::SetSourceCellId(uint16_t sourceCellId)
{
...
uint32_t
EpcX2RlfIndicationMessageHeader::GetLengthOfIes () const
{
	return m_headerLength;
}

uint32_t
EpcX2RlfIndicationMessageHeader::GetNumberOfIes () const
{
	return m_numberOfIes;
}

uint64_t
EpcX2RlfIndicationMessageHeader::GetNewTeId() const
{
	return m_newTeId;
}

void
EpcX2RlfIndicationMessageHeader::SetNewTeId(uint64_t newTeId)
{
	m_newTeId = newTeId;
}

void
EpcX2RlfIndicationMessageHeader::SetMeasResultEutra(std::list<LteRrcSap::MeasResultEutra> rlfMeasData)
{
	m_headerLength += (2+(2+2+2)*rlfMeasData.size());//#element + Total Size
	m_rlfMeasData = rlfMeasData;
}

std::list<LteRrcSap::MeasResultEutra>
EpcX2RlfIndicationMessageHeader::GetMeasResultEutra() const
{
	return m_rlfMeasData;
}
