...
//  /******************THANG*****************/
//  void DoReleaseListIndication (std::list<uint8_t> bearerId, uint64_t imsi, uint16_t rnti);
//  /****************************************/
  /**
...
public:
   ...
   * SON API
   */
  void EnbS1apRcvPMReportNotify(uint16_t cellId, SONCommon::SONMsgInfo msgInfo, SONCommon::PerformanceReport pmReport);
  /*
   * Receive new Params from Mme
   */
  void EnbS1apRecvUpdateParamConfig(SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList);
  /*
   * Receive DSON Parameters Request from Mme
   * Forward it to RRC
   */
  void EnbS1apRecvDSONParamReq(SONCommon::SONMsgInfo msgInfo);
  void EnbS1apRecvDSONParamResp(uint16_t cellId, SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList);
}; //End EpcEnbApplication
