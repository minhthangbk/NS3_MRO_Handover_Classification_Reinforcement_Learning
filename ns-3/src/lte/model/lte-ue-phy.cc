...
//    m_cellload((double)1/6),
//    m_SINRinsync(4), //db
//    m_SINRoutofsync(2), //db
//    m_RSRQinsync(-10*log10(12*(this->m_cellload + pow(10,-(this->m_SINRinsync)/10)))), //-8.31
//    m_RSRQoutofsync(-10*log10(12*(this->m_cellload + pow(10,-(this->m_SINRoutofsync)/10)))), //-9.8
//    m_outofsyncFlag(false)
////    m_firstWriteNewMeasurement(true)
//    /*************************************/
{
...
//  m_RSRQinsync = -10;
//  m_RSRQoutofsync = -11;
//  NS_ASSERT(m_RSRQoutofsync < m_RSRQinsync);
  ...
  static TypeId tid = TypeId ("ns3::LteUePhy")
    ...
    .AddAttribute ("Qoutofsync",
            "Out of sync threshold in dB",
             DoubleValue (-10),
             MakeDoubleAccessor (&LteUePhy::SetQoutofsync,&LteUePhy::GetQoutofsync), //MakeDoubleAccessor (&LteUePhy::m_RSRQoutofsync),
             MakeDoubleChecker<double> ())
	 .AddAttribute ("Qinsync",
			 "In sync threshold in dB",
			  DoubleValue (-9),
			  MakeDoubleAccessor (&LteUePhy::SetQinsync,&LteUePhy::GetQinsync), //MakeDoubleAccessor (&LteUePhy::m_RSRQinsync)
			  MakeDoubleChecker<double> ())
	 .AddAttribute ("RAT",
			  "Resource Allocation Type",
			   UintegerValue(0),
			   MakeUintegerAccessor(&LteUePhy::m_rat),
...
void
LteUePhy::SetQinsync(double q){
	NS_LOG_LOGIC("LteUePhy::SetQinsync " << q);
	m_RSRQinsync = q;
}

double
LteUePhy::GetQinsync() const{
	return m_RSRQinsync;
}

void
LteUePhy::SetQoutofsync(double q){
	NS_LOG_LOGIC("LteUePhy::SetQoutofsync " << q);
	m_RSRQoutofsync = q;
}

double
LteUePhy::GetQoutofsync() const{
	return m_RSRQoutofsync;
}
...
LteUePhy::GenerateCqiRsrpRsrq (const SpectrumValue& sinr)
{
	...
	if (m_rsrpSinrSampleCounter==m_rsrpSinrSamplePeriod)
	{
		...
		m_reportCurrentCellRsrpSinrTrace (m_cellId, m_rnti, rsrp, 10*std::log10(avSinr));
		//Create a Notify function to RRC!!!
		m_rsrpSinrSampleCounter = 0;
	}
...
void
LteUePhy::GenerateMixedCqiReport (const SpectrumValue& sinr) //Call from LteSpectrumPhy
{
	NS_LOG_FUNCTION (this);
	...
	{
		m_dataInterferencePowerUpdated = false;
		if(mixedSinr.GetSpectrumModelUid() == m_dataInterferencePower.GetSpectrumModelUid())
		{
	...
	{
		NS_FATAL_ERROR("The RAT is not implemented yet: " << (uint32_t)m_rat);
	}
...
LteUePhy::CreateDlCqiFeedbackMessage (const SpectrumValue& sinr)
{
	...
	if (Simulator::Now () > m_p10CqiLast + m_p10CqiPeriocity)
	{
		...
		for (int i = 0; i < nbSubChannels; i++)
		{
			...
			NS_LOG_INFO (this << " subch " << i << " cqi " <<  cqi.at (i));
		}
		dlcqi.m_rnti = m_rnti;
		...
		else if(m_rat == 2)
		{
			for (int i = 0; i < nbSubChannels; i++)
			{
				if (cqi.at (i) != -1)
				{
					...
					for (int i = 0; i < nLayer; i++)
					{
						hlCqi.m_sbCqi.push_back (cqi.at (i));
					}
					rbgMeas.m_higherLayerSelected.push_back (hlCqi);
				}
			}
...
//	NS_LOG_UNCOND("LteUePhy::ReportUeMeasurements: Report UE Measurements NumSamples=" << (uint16_t)(*m_ueMeasurementsMap.begin ()).second.rsrpNum);
	...
	for (it = m_ueMeasurementsMap.begin (); it != m_ueMeasurementsMap.end (); it++)
	{
		...
		if(((*it).first == m_cellId) && (m_cellId > 0)){ //at primary cell
			if(avg_rsrq < m_RSRQoutofsync){
				m_outofsyncFlag = true; //add flag + SINR to measurementsReport ???
			}else if (avg_rsrq > m_RSRQinsync){
				m_outofsyncFlag = false; //add flag + SINR to measurementsReport ???
			}
		}else{
			m_outofsyncFlag = false;
		}
		newEl.m_outofsyncFlag = m_outofsyncFlag; //update sync status to measurement report to RRC layer
...
LteUePhy::ReceiveLteControlMessageList (std::list<Ptr<LteControlMessage> > msgList)
{
	...
	for (it = msgList.begin (); it != msgList.end (); it++)
	{
		...
		if (msg->GetMessageType () == LteControlMessage::DL_DCI)
		{
			...
			case 0:
			{
				...
				for (int i = 0; i < 32; i++)
				{
					if (((dci.m_rbBitmap & mask) >> i) == 1)
					{
						...
						{
							dlRb.push_back ((i * GetRbgSize ()) + k); //indexing N RBs: 0th...(N-1)th
			...
			case 1:
				NS_FATAL_ERROR ("This RAT " << (uint32_t)dci.m_resAlloc << " is not implemented yet");
				break;

			case 2:
			{
				uint8_t startRb = dci.m_rbRIV % m_dlBandwidth;
				uint8_t rbNum = dci.m_rbRIV / m_dlBandwidth + 1;
				if((startRb + rbNum) > m_dlBandwidth)
				{
					startRb = m_dlBandwidth - 1 - startRb;
					rbNum = m_dlBandwidth + 2 - rbNum;
				}
				for (uint8_t i = 1; i <= rbNum; i++)
				{
					dlRb.push_back(startRb + i - 1);
					NS_LOG_DEBUG(this << " RNTI " << m_rnti << " DL-DCI allocated PRB " << (uint32_t)(startRb + i - 1));
				}
			}
			break;
			...
			NS_LOG_INFO (this << " UE " << m_rnti << " DL-DCI " << dci.m_rnti << " bitmap "  << dci.m_rbBitmap);
...
void
LteUePhy::ReceivePss (uint16_t cellId, Ptr<SpectrumValue> p) //physical spectrum
{
...
//	NS_LOG_UNCOND("LteUePhy::ReceivePss nRB: " << nRB);
...
LteUePhy::SubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
	...
	if (m_ulConfigured)
	{
		...
		if (m_srsConfigured && (m_srsStartTime <= Simulator::Now ()))
		{
			...
			{
				NS_LOG_LOGIC("LteUePhy::SubframeIndication RNTI " << m_rnti << " frame " << frameNo <<
						" cellId " << m_cellId << " subframe " << subframeNo <<  " SRS offset= "
						<< m_srsSubframeOffset << " period " << m_srsPeriodicity);
		...
		if (pb)
		{
			...
			NS_LOG_LOGIC("LteUePhy::SubframeIndication StartTxDataFrame in PUSCH from IMSI "<< m_imsi << " to cellId " << m_cellId <<
					" at frame " << frameNo << " subframe " << subframeNo);// << " #Packets " << pb->GetNPackets());
			m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, UL_DATA_DURATION);//Call LteSpectrumPhy
		}
		else
		{
			...
			if (ctrlMsg.size ()>0)
			{
				NS_LOG_LOGIC("LteUePhy::SubframeIndication UE - start TX PUCCH (NO PUSCH) from IMSI " << m_imsi <<" to cellId " << m_cellId);
				std::vector <int> dlRb;
...
{
	NS_LOG_DEBUG ("LteUePhy::DoSynchronizeWithEnb " << cellId);

	if (cellId == 0)
	{
		NS_FATAL_ERROR ("Cell ID shall not be zero");
	}
	//  if(m_srsConfigured == true){
	m_srsConfigured = false;
	//  }
	//  if(m_sendSrsEvent.IsRunning()){
	m_sendSrsEvent.Cancel();//prevent SRS collision, need for RLF
	//  }
...
LteUePhy::DoSetSrsConfigurationIndex (uint16_t srcCi)
{
	...
	NS_LOG_INFO (this << " UE SRS P " << m_srsPeriodicity << " RNTI " << m_rnti << " offset " << m_srsSubframeOffset << " cellId " << m_cellId << " CI " << srcCi);
}
