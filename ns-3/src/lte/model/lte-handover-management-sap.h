...
public:
   ...
   * THANG Add cellId for logging
   */
  virtual void ReportUeMeas (uint16_t cellId, uint16_t imsi, uint16_t rnti, LteRrcSap::MeasResults measResults) = 0;
...
public:
  ...
  virtual void ReportUeMeas (uint16_t cellId, uint16_t imsi, uint16_t rnti, LteRrcSap::MeasResults measResults);
...
template <class C>
void
MemberLteHandoverManagementSapProvider<C>::ReportUeMeas (uint16_t cellId, uint16_t imsi, uint16_t rnti, LteRrcSap::MeasResults measResults)
{
  m_owner->DoReportUeMeas (cellId, imsi, rnti, measResults);
}
