...
public:
  ...
  /*
   * THANG
   * add methods supporting IMSI
   */
  uint64_t GetUeIMSI () const;
  void SetUeIMSI(uint64_t imsi);
...
private:
  ...
  /*
   * THANG
   * add imsi for pingpong Detection
   */
  uint64_t m_imsi;
...
/**********************THANG**********************/
class EpcX2RlfIndicationMessageHeader: public Header
{
public:
	EpcX2RlfIndicationMessageHeader();
	virtual ~EpcX2RlfIndicationMessageHeader();
	...
	uint16_t GetFailureCellId () const; //for receiving
	void SetFailureCellId (uint16_t failureCellId);

	uint16_t GetReportCellId () const;
	void SetReportCellId (uint16_t reportCellId);

	uint16_t GetFailureCRNTI () const;
	void SetFailureCRNTI (uint16_t failureCRNTI);
	...
	uint64_t GetFailureIMSI () const;
	void SetFailureIMSI (uint64_t imsi);

	uint64_t GetNewTeId () const;
	void SetNewTeId (uint64_t newTeDd);

	void SetMeasResultEutra(std::list<LteRrcSap::MeasResultEutra> rlfMeasData);
	std::list<LteRrcSap::MeasResultEutra> GetMeasResultEutra() const;
...
private:
	  ...
	  //message contents
	  uint16_t m_failureCRNTI;
	  uint64_t m_failureIMSI;
	  uint16_t m_failureCellId;
	  uint16_t m_reportCellId;
