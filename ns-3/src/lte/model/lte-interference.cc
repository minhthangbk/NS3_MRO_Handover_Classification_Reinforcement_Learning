...
{ 
  ...
  if (m_receiving == false)
  ...
  else
    {
      ...
      /*
       *
this assert fails when for whatever reason two uplink transmissions to the same eNB have power in the same resource block (which should not happen).
a bug that caused the SRS configuration of different UEs in the same cell to be not consistent.
       */
//      NS_LOG_UNCOND("LteInterference::StartRx " << Sum ((*rxPsd) * (*m_rxSignal)));

//      double s1 = 0;
//      for(Values::const_iterator it1 = (*rxPsd).ConstValuesBegin(); it1 != (*rxPsd).ConstValuesEnd(); ++it1){
//    	  s1 += (*it1);
//      }
//      double s2 = 0;
//      for(Values::const_iterator it2 = (*m_rxSignal).ConstValuesBegin(); it2 != (*m_rxSignal).ConstValuesEnd(); ++it2){
//          s2 += (*it2);
//      }
//      NS_LOG_UNCOND("LteInterference::StartRx s1 " << s1 << " s2 " << s2 << " Sum " << Sum ((*rxPsd) * (*m_rxSignal))
//    		  << " frequency set id " << rxPsd->GetSpectrumModel()->GetUid() << " #Bands " << rxPsd->GetSpectrumModel()->GetNumBands());

      //bug error SUM = nan: rxPsd = nan m_rxSignal != nan
      if(Sum ((*rxPsd) * (*m_rxSignal)) != 0.0)
      {
    	  NS_LOG_UNCOND("Spectrum allocation error, LteInterference::StartRx SpectrumId: " << m_rxSignal->GetSpectrumModelUid()
    			  << " #Bands: "<< m_rxSignal->GetSpectrumModel()->GetNumBands()
    			  << " rxSpectrumId: " << rxPsd->GetSpectrumModelUid()
    			  << " #rxBands: " <<rxPsd->GetSpectrumModel()->GetNumBands());
      }
//      NS_ASSERT (Sum ((*rxPsd) * (*m_rxSignal)) == 0.0);//SRS collision due to number of UEs-> increase SRS period
      (*m_rxSignal) += (*rxPsd);
    }
}
