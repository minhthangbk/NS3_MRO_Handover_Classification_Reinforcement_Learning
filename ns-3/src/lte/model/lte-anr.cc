...
LteAnr::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::LteAnr")
    ...
    .AddAttribute ("Threshold",
                   "Minimum RSRP range value required for detecting a neighbour cell",
                   UintegerValue (0),
                   MakeUintegerAccessor (&LteAnr::m_threshold),
                   MakeUintegerChecker<uint8_t> (0, 34)) // Thang changes from [0..34] (RSRQ) to [0..97] RSRP (following as Section 9.1.7 of 3GPP TS 36.133)
  ;
  return tid;
}
...
LteAnr::DoInitialize ()
{
  ...
  reportConfig.threshold1.choice = LteRrcSap::ThresholdEutra::THRESHOLD_RSRP; //THANG changes to THRESHOLD_RSRP, previous one is THRESHOLD_RSRQ
  reportConfig.threshold1.range = m_threshold;
  reportConfig.triggerQuantity = LteRrcSap::ReportConfigEutra::RSRP; //THANG changes to RSRP, previous one is RSRQ
  reportConfig.reportInterval = LteRrcSap::ReportConfigEutra::MS480; //THANG changes from 480ms to 240MS
