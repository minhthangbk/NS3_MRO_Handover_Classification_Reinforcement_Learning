  ...
  static TypeId tid = TypeId ("ns3::CqaFfMacScheduler")
	...
	.AddAttribute ("RAT",
				   "Resource Allocation Type",
				    UintegerValue (0),
				   	MakeUintegerAccessor (&CqaFfMacScheduler::m_rat),
				   	MakeUintegerChecker<uint8_t> ())
  ;
  return tid;
}
...
CqaFfMacScheduler::DoSchedDlRlcBufferReq (const struct FfMacSchedSapProvider::SchedDlRlcBufferReqParameters& params)
{
	...
	{
//		(*it).second = params; //THANG
		//THANG: consider HARQ later
		(*it).second.m_rlcTransmissionQueueSize += params.m_rlcTransmissionQueueSize;
		(*it).second.m_rlcTransmissionQueueHolDelay += params.m_rlcTransmissionQueueHolDelay;
	}
...
CqaFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
	...
	std::vector <bool> rat0RbgMap;  // global RBGs map
	uint16_t rat0RbgAllocatedNum = 0;
	std::set <uint16_t> rntiAllocated;
	rat0RbgMap.resize (m_cschedCellConfig.m_dlBandwidth / rbgSize, false);

	rat0RbgMap = m_ffrSapProvider->GetAvailableDlRbg ();
	for (std::vector<bool>::iterator it = rat0RbgMap.begin (); it != rat0RbgMap.end (); it++)
	{
		if ((*it) == true )
		{
			rat0RbgAllocatedNum++;
	...
	std::vector <bool> rat2RbMap;//THANG
	uint8_t rat2RbAllocatedNum = 0;
	for(uint8_t i = 0; i < m_cschedCellConfig.m_dlBandwidth; i++)
	{
		rat2RbMap.push_back(false); //reset RA status
	}
	...
	for (uint16_t i = 0; i < m_dlInfoListBuffered.size (); i++)
	{
		...
		if (retx.at (0) || retx.at (1))
		{
			...
			if(m_rat == 0)//THANG
			{
				...
				dci.m_resAlloc = 0; //RAT 0
				for (int j = 0; j < 32; j++)
				{
					if (((dci.m_rbBitmap & mask) >> j) == 1) // right to left ~ increasing order -> LSB ~ ending RBG ID
					{
						dciRbg.push_back (j); // translate the DCI RB Bitmap to sequence of RB in the spectrum
						NS_LOG_INFO ("\t" << j);
					}
					mask = (mask << 1);
				}
				...
				{
					if (rat0RbgMap.at (dciRbg.at (j)) == true)
					{
						free = false;
						break; //break if find any allocated RBG
					}
				}
				if (free)
				{//if the resource is free -> use the same RBGs for the retransmission -> reserve RBGs
					for (uint8_t j = 0; j < dciRbg.size (); j++)
					{
						rat0RbgMap.at (dciRbg.at (j)) = true;
				...
				{//if the resource is partially assigned before
					// find RBGs for sending HARQ retransmission
					uint8_t j = 0;
					//try the second RBG Id, because the the previous loop stops when at least first RBG is not free
					uint8_t rbgId = (dciRbg.at (dciRbg.size () - 1) + 1) % numberOfRBGs;
					uint8_t startRbg = dciRbg.at (dciRbg.size () - 1); //last element of DCI RBG is the starting RBG Id(because right to left is increasing order)
					std::vector <bool> rbgMapCopy = rat0RbgMap;
					while ((j < dciRbg.size ())&&(startRbg != rbgId))
					{//the loop is stopped when all required RBGs are allocated and
						//#allocated RBG don't exceed the available #RBG
						if (rbgMapCopy.at (rbgId) == false)
						{
							rbgMapCopy.at (rbgId) = true; //assign corresponding bit
							dciRbg.at (j) = rbgId; //assign corresponding rbgId
							j++;
						}
						rbgId = (rbgId + 1) % numberOfRBGs; //increase the index
					}
					if (j == dciRbg.size ())
					{//all required RBGs are allocated
						uint32_t rbgMask = 0;
						...
						dci.m_rbBitmap = rbgMask; //update DCI map for this HARQ retransmission
						rat0RbgMap = rbgMapCopy; //update overall RBG map
						NS_LOG_INFO (this << " Move retx in RBGs " << dciRbg.size ());
					}
					else
					{// HARQ retransmission cannot be performed on this TTI -> store it for later schedule period
						dlInfoListUntxed.push_back (m_dlInfoListBuffered.at (i));
						NS_LOG_INFO (this << " No resource for this retx -> buffer it");
					}
				}
			}//End if RAT = 0
			else if(m_rat == 2)//THANG
			{//TODO: Implementing RAT2 for HARQ

				dci.m_resAlloc = 2; //RAT 2
				uint8_t rat2DciStartingRbId = dci.m_rbRIV % m_cschedCellConfig.m_dlBandwidth;
				uint8_t rat2DciRbNum = dci.m_rbRIV / m_cschedCellConfig.m_dlBandwidth + 1;
				if((rat2DciStartingRbId + rat2DciRbNum) > m_cschedCellConfig.m_dlBandwidth)
				{//Recalculation
					rat2DciStartingRbId = m_cschedCellConfig.m_dlBandwidth - 1 - rat2DciStartingRbId;
					rat2DciRbNum = m_cschedCellConfig.m_dlBandwidth + 2 - rat2DciRbNum;
				}

				std::vector<uint8_t> rat2DciRbs;
				for(uint8_t j = 0; j < rat2DciRbNum; j++)
				{
					rat2DciRbs.push_back(rat2DciStartingRbId+j); //translate to RB sequences in spectrum
				}
				bool free = true;
				for(uint8_t j = 0; j < rat2DciRbNum; j++)
				{
					if(rat2RbMap.at(rat2DciRbs.at(j)) == true)
					{
						free = false;
						break;
					}
				...
				if(free)
				{//all RBs allocated for this HARQ are free -> reallocate them now
					for(uint8_t j = 0; j < rat2DciRbNum; j++)
					{
						rat2RbMap.at(rat2DciRbs.at(j)) = true;
						rat2RbAllocatedNum++;
					}
				}
				else
				{//find other RBs for sending HARQ retx
					uint8_t j = 0;
					uint8_t rat2RbId = (rat2DciStartingRbId + 1) % m_cschedCellConfig.m_dlBandwidth;
					std::vector<bool> rat2RbMapCopy = rat2RbMap;
					while((j < rat2DciRbNum) && (rat2RbId != rat2DciStartingRbId))
					{
						if(rat2RbMapCopy.at(rat2RbId) == false)
						{
							rat2RbMapCopy.at(rat2RbId) = true;
							rat2DciRbs.at(j) = rat2RbId;
							j++;
						}
						rat2RbId = (rat2RbId+1) % m_cschedCellConfig.m_dlBandwidth;
					}
					if(j == rat2DciRbNum)
					{
						rat2RbMap = rat2RbMapCopy;//Marked as being allocated
						if((rat2DciRbNum-1) <= m_cschedCellConfig.m_dlBandwidth/2)
						{//calculate RIV (Resource Allocation Value)
							dci.m_rbRIV = m_cschedCellConfig.m_dlBandwidth*(rat2DciRbNum-1)+rat2DciRbs.at(0);
						}
						else
						{
							dci.m_rbRIV = m_cschedCellConfig.m_dlBandwidth*(m_cschedCellConfig.m_dlBandwidth-rat2DciRbNum+1)
																+ m_cschedCellConfig.m_dlBandwidth - 1 - rat2DciRbs.at(0);
						}

					}
					else
					{//This HARQ process cannot be performed at this TTT -> store it for next scheduling event
						dlInfoListUntxed.push_back(m_dlInfoListBuffered.at(i));
					}
				}
			}//End if RAT = 2
			...
			{
				NS_FATAL_ERROR("This RAT " << (uint32_t)m_rat << " is not implemented");
			}
	...
	//	NS_LOG_UNCOND("CQA Scheduler m_rlcBufferReq: " << m_rlcBufferReq.size());
			...
			itrbr!=m_rlcBufferReq.end (); itrbr++)
	{
		...
		// map: UE, to the amount of traffic they have to transfer
		//		NS_LOG_UNCOND("RNTI: " << flowId.m_rnti << " rlcRetransmissionQueueSize: " << (int)m_rlcBufferReq.find (flowId)->second.m_rlcRetransmissionQueueSize
		//				<<" rlcTransmissionQueueSize: " << (int)m_rlcBufferReq.find (flowId)->second.m_rlcTransmissionQueueSize);
	...
	}//end while m_rlcBufferReq

	// availableRes - set that contains indexes of available resource block groups
	std::set<int> availableRes;
	if(m_rat == 0)//THANG
	{
		for (int i = 0; i <  numberOfRBGs; i++)
		{
			if (rat0RbgMap.at (i) == false)
			{
				availableRes.insert (i);
			}
		}
	}
	else if(m_rat == 2)//THANG
	{
		for (int i = 0; i <  m_cschedCellConfig.m_dlBandwidth; i++)
		{
			if (rat2RbMap.at (i) == false)
			{
				availableRes.insert (i);
			}
		}
	...
	{
		NS_FATAL_ERROR("This RAT " << (uint32_t)m_rat << " is not implemented yet");
	}
	...
	while (availableRes.size ()>0)
	{
		...
		{
			NS_LOG_INFO ("Available RBGs:"<< availableRes.size ()<<"but no users");
			break;
		}

		//		if(m_cellId == 1)
		//		{
		//			NS_LOG_UNCOND("cellId: " << m_cellId << " availableRes " << availableRes.size() << " itCurrentGroup->second.size () "
		//					<< itCurrentGroup->second.size () << " before allocation");
		//		}

		while (availableRes.size ()>0 and itCurrentGroup->second.size ()>0)
		{
			bool currentRBchecked = false;
			int currentRB = *(availableRes.begin ());
			...
			for (std::set<LteFlowId_t>::iterator it=itCurrentGroup->second.begin (); it!=itCurrentGroup->second.end (); it++)
			{
				...
				int numberOfResourceAllocatedForThisUser = 0;
				...
				{
					for(std::set<int>::iterator it=availableRes.begin (); it!=availableRes.end (); it++)
					{
						try
						{
				...
				if (allocationMapPerRntiPerLCId.find (flowId.m_rnti)==allocationMapPerRntiPerLCId.end ())
				{
					...
					//					if(m_cellId == 1)
					//						NS_LOG_UNCOND("cellId: " << m_cellId << " RNTI: " << flowId.m_rnti << " cqi_value "<<(uint32_t)cqi_value);

				}
				else
				{
					numberOfResourceAllocatedForThisUser = (allocationMapPerRntiPerLCId.find (flowId.m_rnti)->second.size ());

					for (std::multimap <uint8_t, qos_rb_and_CQI_assigned_to_lc>::iterator itRbCqiLc = allocationMapPerRntiPerLCId.find (flowId.m_rnti)->second.begin ();
							itRbCqiLc!=allocationMapPerRntiPerLCId.find (flowId.m_rnti)->second.end (); itRbCqiLc++)
					{
						qos_rb_and_CQI_assigned_to_lc e = itRbCqiLc->second;
					...
					//					if(m_cellId == 1)
					//						NS_LOG_UNCOND("cellId: " << m_cellId << " RNTI: " << flowId.m_rnti
					//								<< " numberOfResourceAllocatedForThisUser: " << numberOfResourceAllocatedForThisUser
					//								<< " worstCQIAmongRBGsAllocatedForThisUser: " << (uint32_t)worstCQIAmongRBGsAllocatedForThisUser);

				}

				//				NS_LOG_UNCOND("RNTI: " << flowId.m_rnti << " worstCQIAmongRBGsAllocatedForThisUser " <<(uint32_t)worstCQIAmongRBGsAllocatedForThisUser);
				int mcsForThisUser = m_amc->GetMcsFromCqi (worstCQIAmongRBGsAllocatedForThisUser);

				uint8_t requiredRbNum = 0;
				double achievableRate = 0;
				int tbSize = 0;
				if(m_rat == 0)//THANG
				{
					requiredRbNum = (numberOfResourceAllocatedForThisUser+1) * rbgSize;
					achievableRate = (( m_amc->GetTbSizeFromMcs (mcsForThisUser, rbgSize)/ 8) / 0.001);
					tbSize = m_amc->GetTbSizeFromMcs (mcsForThisUser, requiredRbNum)/8;

					//					if(m_cellId == 1)
					//						NS_LOG_UNCOND("cellId: " << m_cellId << " RNTI: " << flowId.m_rnti << " mcsForThisUser: " << mcsForThisUser
					//								<< " requiredRbNum: " << (uint32_t)requiredRbNum << " achievableRate: " << achievableRate << " tbSize: " << tbSize);
				}
				else if(m_rat == 2)//THANG
				{
					requiredRbNum = (numberOfResourceAllocatedForThisUser == 0) ? 1 : numberOfResourceAllocatedForThisUser;
					achievableRate = (( m_amc->GetTbSizeFromMcs (mcsForThisUser, 1)/ 8) / 0.001);
					tbSize = m_amc->GetTbSizeFromMcs (mcsForThisUser, requiredRbNum)/8;
				...
				{
					NS_FATAL_ERROR("This RAT " << (uint32_t)m_rat << " is not implemented yet");
				}                          // similar to calculation of TB size (size of TB in bytes according to table 7.1.7.2.1-1 of 36.213)

				double pf_weight = achievableRate / (*itStats).second.secondLastAveragedThroughput;
				UeToAmountOfAssignedResources.find (flowId)->second += tbSize; //= tbSize
				...
				if (metric >= maximumValueMetric)//find user with highest metric
				{
					//					if(m_cellId == 1)
					//					{
					//						NS_LOG_UNCOND("CellId: " << m_cellId << " RNTI: " << flowId.m_rnti << " metric: " << metric << " maxMetric: " << maximumValueMetric
					//								<< " coita_metric: " << coita_metric << " tbr_weight: " << tbr_weight << " hol: " << hol <<
					//								" targetThroughput: " << (*itStats).second.targetThroughput <<
					//								" lastAveragedThroughput: " << (*itStats).second.lastAveragedThroughput);
					//					}

					maximumValueMetric = metric;
					userWithMaximumMetric = flowId;
				}
				//if metric > maximumValueMetric && if metric == maximumValueMetric ???
			}//end FOR

			if (!currentRBchecked)
			{
				// erase current RBG from the list of available RBG
				availableRes.erase (currentRB);
				continue;
			}
			...
			if (itMap == allocationMapPerRntiPerLCId.end ())
			{
				...
				allocationMapPerRntiPerLCId.insert (std::pair <uint16_t,
						std::multimap <uint8_t,qos_rb_and_CQI_assigned_to_lc> > (userWithMaximumMetric.m_rnti, tempMap));
				//				if(m_cellId == 1)
				//				{
				//					NS_LOG_UNCOND("cellId: " << m_cellId << " allocationMapPerRntiPerLCId add RNTI: " << userWithMaximumMetric.m_rnti
				//							<< " LcId: " << (uint32_t)userWithMaximumMetric.m_lcId);
				//				}
			}
			else
			{
				...
				//				if(m_cellId == 1)
				//				{
				//					NS_LOG_UNCOND("cellId: " << m_cellId << " allocationMapPerRntiPerLCId update RNTI: " << userWithMaximumMetric.m_rnti
				//							<< " LcId: " << (uint32_t)userWithMaximumMetric.m_lcId);
				//				}
			}
			...
			//			NS_LOG_UNCOND("RNTI WithMaximumMetric: " << userWithMaximumMetric.m_rnti << " AmountOfDataToTransfer: "
			//					<< UeToAmountOfDataToTransfer.find (userWithMaximumMetric)->second << " AmountOfAssignedResources*tolerance "
			//					<< 8*UeToAmountOfAssignedResources.find (userWithMaximumMetric)->second*tolerance);
			if (UeToAmountOfDataToTransfer.find (userWithMaximumMetric)->second <= 8*UeToAmountOfAssignedResources.find (userWithMaximumMetric)->second*tolerance)
				//||(UeHasReachedGBR.find(userWithMaximumMetric)->second == true))
			{
				//				NS_LOG_UNCOND("Erase RNTI: " << userWithMaximumMetric.m_rnti << " LcId: " << (uint32_t)userWithMaximumMetric.m_lcId);
		...
		//		{
		//			NS_LOG_UNCOND("cellId: " << m_cellId << " availableRes " << availableRes.size() << " itCurrentGroup->second.size () "
		//					<< itCurrentGroup->second.size () << " after allocation");
		//		}
	...
	while (itMap != allocationMapPerRntiPerLCId.end ())
	{
		...
		if(m_rat == 0)//THANG
		{//RBG-based scheduling (continuous allocation)
			newDci.m_resAlloc = 0;
		...
		{//RB-based scheduling
			newDci.m_resAlloc = 2;
			uint16_t RbPerRnti = (*itMap).second.size ();
			//			std::map <uint16_t,SbMeasResult_s>::iterator itCqi;
			//			itCqi = m_a30CqiRxed.find ((*itMap).first);
			uint8_t worstCqi = 15;
			...
			newDci.m_mcs.push_back (m_amc->GetMcsFromCqi (worstCqi));
			int tbSize = (m_amc->GetTbSizeFromMcs (newDci.m_mcs.at (0), RbPerRnti) / 8);           // (size of TB in bytes according to table 7.1.7.2.1-1 of 36.213)

			newDci.m_tbsSize.push_back (tbSize);

			uint8_t perRNTIRbNum = (*itMap).second.size();
			uint8_t startRbId = (*itMap).second.begin ()->second.resource_block_index;
			if((perRNTIRbNum-1) <= (m_cschedCellConfig.m_dlBandwidth / 2))
			{
				newDci.m_rbRIV = m_cschedCellConfig.m_dlBandwidth*(perRNTIRbNum-1) + startRbId;
			}
			else
			{
				newDci.m_rbRIV = m_cschedCellConfig.m_dlBandwidth*(m_cschedCellConfig.m_dlBandwidth-perRNTIRbNum+1)+(m_cschedCellConfig.m_dlBandwidth-startRbId-1);
			}
	...
	if (it != m_rlcBufferReq.end ())
	{
		...
		else if ((*it).second.m_rlcTransmissionQueueSize > 0)
		{
			...
			if ((*it).second.m_rlcTransmissionQueueSize <= size - rlcOverhead)
			{
				...
				(*it).second.m_rlcTransmissionQueueHolDelay = 0; //THANG
			}
			else
			{
