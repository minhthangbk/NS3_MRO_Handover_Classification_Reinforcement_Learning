  ...
  static TypeId tid = TypeId ("ns3::LteEnbPhy")
	...
	.AddAttribute("RAT",
	    		  "Resource Allocation Type",
				   UintegerValue(0),
				   MakeUintegerAccessor (&LteEnbPhy::m_rat),
				   MakeUintegerChecker<uint8_t> ())
  ;
  return tid;
}
...
//  NS_LOG_UNCOND("LteEnbPhy::SetTxPower " << pow);//THANG for debug
  m_txPower = pow;
}
...
LteEnbPhy::StartSubFrame (void)
{
	...
	if (ctrlMsg.size () > 0)
	{
		...
		while (it != ctrlMsg.end ())
		{
			...
			if (msg->GetMessageType () == LteControlMessage::DL_DCI)
			{
				...
				{
					uint32_t mask = 0x1;
					for (int i = 0; i < 32; i++)//Actually 25 is enough
					{//
				...
				{//Calculate from RIV
					uint8_t startRb = (dci->GetDci().m_rbRIV) % m_dlBandwidth;
					uint8_t RbNum = (dci->GetDci().m_rbRIV) / m_dlBandwidth + 1;
					...
					for(uint8_t i = 0; i < RbNum; i++)
					{
						m_dlDataRbMap.push_back(startRb + i);
						GeneratePowerAllocationMap(dci->GetDci().m_rnti,startRb + i);
					}
				}
				else//THANG
				{
	...
	// trigger the MAC layer (DoSubframeIndication)
	NS_LOG_LOGIC("LteEnbPhy::StartSubFrame trigger SubframeIndication at cellId " << m_cellId << " Frame "
			<< m_nrFrames << " Subframe " << m_nrSubFrames);
...
LteEnbPhy::SendDataChannels (Ptr<PacketBurst> pb)
{
  ...
  ctrlMsgList.clear ();
  NS_LOG_LOGIC("LteEnbPhy::SendDataChannels  StartTxDataFrame in downlink ");
...
//  NS_LOG_UNCOND("LteEnbPhy::DoSetSrsConfigurationIndex cellId " << m_cellId << " RNTI " << rnti << " srcCi " << srcCi); //For debugging
