...
{
  NS_LOG_FUNCTION ("DoSendSystemInformation " << "Cell ID" << m_cellId); //THANG
          ...
          if (ueDev != 0)
            {
              ...
              if (ueRrc->GetCellId () == m_cellId) //looking for Ue which associates with the enb
                {
                  NS_LOG_INFO ("LteEnbRrcProtocolIdeal::DoSendSystemInformation CellId " << m_cellId << " sending SI to IMSI " << ueDev->GetImsi ()); //THANG: only for debug
                  ueRrc->GetLteUeRrcSapProvider ()->RecvSystemInformation (msg); //Let a Ue Receiving SIB
                  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, //Re-schedule because of delay ???
