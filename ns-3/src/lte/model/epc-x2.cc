...
EpcX2::RecvFromX2cSocket (Ptr<Socket> socket)
{
  ...
  if (procedureCode == EpcX2Header::HandoverPreparation)
    {
      if (messageType == EpcX2Header::InitiatingMessage)
      ...
      else if (messageType == EpcX2Header::SuccessfulOutcome)
        {
           ...
           * THANG add IMSI
           */
          params.imsi = x2HoReqAckHeader.GetUeIMSI();
  ...
  /*****************THANG*****************/
  else if (procedureCode == EpcX2Header::RlfIndication){
      if(messageType == EpcX2Header::InitiatingMessage){
    	  NS_LOG_LOGIC ("Recv X2 message: RESOURCE STATUS UPDATE");

		  EpcX2RlfIndicationMessageHeader x2RlfMsgHeader;
		  packet->RemoveHeader (x2RlfMsgHeader);

		  NS_LOG_INFO ("X2 RLF Indication header: " << x2RlfMsgHeader);

		  EpcX2SapUser::RlfIndicationMessageParams params;
		  params.failureCRNTI = x2RlfMsgHeader.GetFailureCRNTI();
		  params.failureIMSI = x2RlfMsgHeader.GetFailureIMSI();
		  params.failureCellId = x2RlfMsgHeader.GetFailureCellId();
		  params.reportCellId = x2RlfMsgHeader.GetReportCellId();
          params.forwarded = false;
          params.sourceCellId = x2RlfMsgHeader.GetSourceCellId();
          params.newTeId = x2RlfMsgHeader.GetNewTeId();
          params.rlfMeasData = x2RlfMsgHeader.GetMeasResultEutra();

		  NS_LOG_LOGIC ("failureCRNTI = " << params.failureCRNTI);
		  NS_LOG_LOGIC ("failureCellId = " << params.failureCellId);
		  NS_LOG_LOGIC ("reportCellId = " << params.reportCellId);

		  m_x2SapUser->RecvRlfIndicationMessage(params);
      }else if(messageType == EpcX2Header::ForwardedMsg){
    	  NS_LOG_LOGIC ("Recv X2 message: RESOURCE STATUS UPDATE");
...
EpcX2::DoSendHandoverRequestAck (EpcX2SapProvider::HandoverRequestAckParams params)
{
  ...
  /*
   * THANG add IMSI to header
   */
  x2HoAckHeader.SetUeIMSI(params.imsi);
...
/*******************THANG*********************/
void
EpcX2::DoSendRlfIndicationMessage(EpcX2SapProvider::RlfIndicationMessageParams params){

//	  Ptr<X2IfaceInfo> socketInfo = m_x2InterfaceSockets [params.failureCellId]; //send to rlf cellId
      Ptr<X2IfaceInfo> socketInfo = m_x2InterfaceSockets [params.sourceCellId]; //send to rlf cellId
//      NS_LOG_UNCOND("EpcX2::DoSendRlfIndicationMessage reportCellId " << (uint16_t)params.reportCellId << " failureCellId "
//    		  << (uint16_t)params.failureCellId << " sourceCellId " << (uint16_t)params.sourceCellId << " forwarded " << params.forwarded); //For Debug
	  Ptr<Socket> sourceSocket = socketInfo->m_localCtrlPlaneSocket;
	  Ipv4Address targetIpAddr = socketInfo->m_remoteIpAddr;
	  ...
	  NS_LOG_INFO ("EpcX2::DoSendRlfIndicationMessage ");

	  // Build the X2 message for RLF Indication Message Header
	  EpcX2RlfIndicationMessageHeader x2RlfMsgHeader;
	  x2RlfMsgHeader.SetFailureCRNTI(params.failureCRNTI);
	  x2RlfMsgHeader.SetFailureIMSI(params.failureIMSI);
	  x2RlfMsgHeader.SetFailureCellId(params.failureCellId);
	  x2RlfMsgHeader.SetReportCellId(params.reportCellId);
      x2RlfMsgHeader.SetSourceCellId(params.sourceCellId);
      x2RlfMsgHeader.SetNewTeId(params.newTeId);
      x2RlfMsgHeader.SetMeasResultEutra(params.rlfMeasData);
	  EpcX2Header x2Header;
	  if(params.forwarded)
	  {
		  x2Header.SetMessageType (EpcX2Header::ForwardedMsg); //For wrong cell
	  }
	  else
	  {
		  x2Header.SetMessageType (EpcX2Header::InitiatingMessage);
...
//	  x2Header.SetMessageType (EpcX2Header::InitiatingMessage);
	  x2Header.SetProcedureCode (EpcX2Header::RlfIndication);
	  x2Header.SetLengthOfIes (x2RlfMsgHeader.GetLengthOfIes ());
	  x2Header.SetNumberOfIes (x2RlfMsgHeader.GetNumberOfIes ());
	  ...
	  // Build the X2 packet
	  Ptr<Packet> packet = Create <Packet> ();
	  packet->AddHeader (x2RlfMsgHeader);
	  packet->AddHeader (x2Header);
	  NS_LOG_INFO ("packetLen = " << packet->GetSize ());
