...
EpcSgwPgwApplication::UeInfo::GetTeIdByBearerIdMap()
{
	return m_teidByBearerIdMap;
}

EpcTftClassifier
EpcSgwPgwApplication::UeInfo::GetEpcTftClassifier()
{
	return m_tftClassifier;
}

void
EpcSgwPgwApplication::UeInfo::UpdateTeIdBIDMap(uint8_t bid, uint32_t teid)
{
	m_teidByBearerIdMap[bid] = teid;
}

void
EpcSgwPgwApplication::UeInfo::ClearTftClassifier()
{
	m_tftClassifier.ClearTftMap();
}
...
EpcSgwPgwApplication::RecvFromTunDevice (Ptr<Packet> packet, const Address& source, const Address& dest, uint16_t protocolNumber)
{
	...
	{
		NS_LOG_UNCOND ("EpcSgwPgwApplication::RecvFromTunDevice unknown UE address " << ueAddr);
	}
	else
	{
...
EpcSgwPgwApplication::DoModifyBearerRequest (EpcS11SapSgw::ModifyBearerRequestMessage req)
{
	...
	uint64_t imsi = req.teid; // avoid the need for allocating TEIDs on the S11 interface
	std::map<uint64_t, Ptr<UeInfo> >::iterator ueit = m_ueInfoByImsiMap.find (imsi);
	NS_ASSERT_MSG (ueit != m_ueInfoByImsiMap.end (), "unknown IMSI " << imsi);
	uint16_t cellId = req.uli.gci; //new cell Id
	std::map<uint16_t, EnbInfo>::iterator enbit = m_enbInfoByCellId.find (cellId); //search for new cell's IP address
	NS_ASSERT_MSG (enbit != m_enbInfoByCellId.end (), "unknown CellId " << cellId);
	Ipv4Address enbAddr = enbit->second.enbAddr;
	ueit->second->SetEnbAddr (enbAddr); //replace address with the new one
...
{
	NS_LOG_DEBUG ("EpcSgwPgwApplication::DoDeleteBearerCommand " << req.teid);
