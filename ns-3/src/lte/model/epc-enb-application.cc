...
EpcEnbApplication::DoPathSwitchRequest (EpcEnbS1SapProvider::PathSwitchRequestParameters params)
{
	...
	// side effect: create entry if not exist
	m_imsiRntiMap[imsi] = params.rnti; //Set new RNTI for IMSI
	...
	{
//		EpsFlowId_t flowId; //not used
//		flowId.m_rnti = params.rnti;
//		flowId.m_bid = bit->epsBearerId;

		uint32_t teid = bit->teid;
		EpsFlowId_t rbid (params.rnti, bit->epsBearerId); //Set up new mappings
		// side effect: create entries if not exist
		m_rbidTeidMap[params.rnti][bit->epsBearerId] = teid;
		m_teidRbidMap[teid] = rbid;

		EpcS1apSapMme::ErabSwitchedInDownlinkItem erab; //request sent to epc to switch the path
...
{
	NS_LOG_DEBUG ("EpcEnbApplication::DoUeContextRelease RNTI " << rnti <<" cellID " << m_cellId);
	...
	for(std::map<uint32_t, EpsFlowId_t>::iterator it = m_teidRbidMap.begin(); it != m_teidRbidMap.end();it++){
		EpsFlowId_t snd = it->second;
		NS_LOG_DEBUG("m_teidRbidMap contains gtpTeid " << (uint32_t)(*it).first << " RNTI " << (uint32_t) snd.m_rnti << " BID " << (uint32_t)snd.m_bid << " cellId " <<m_cellId);
	}
}
...
EpcEnbApplication::DoInitialContextSetupRequest (uint64_t mmeUeS1Id, uint16_t enbUeS1Id, std::list<EpcS1apSapEnb::ErabToBeSetupItem> erabToBeSetupList)
{
	for (std::list<EpcS1apSapEnb::ErabToBeSetupItem>::iterator erabIt = erabToBeSetupList.begin ();
			...
			++erabIt)
	{
		NS_LOG_DEBUG ("EpcEnbApplication::DoInitialContextSetupRequest CellId: " << m_cellId << " IMSI: " << mmeUeS1Id
				<<" Bearer QoS: " << erabIt->erabLevelQosParameters.qci);
		...
		m_s1SapUser->DataRadioBearerSetupRequest (params); //interface between EnbRrc and EnbApplication
...
EpcEnbApplication::RecvFromS1uSocket (Ptr<Socket> socket)
{
	...
	{//avoid error when there is no bearer while remote source is sending data, actually there may be packets lost
	...
	{
		NS_LOG_DEBUG("EpcEnbApplication::RecvFromS1uSocket Unknown TeId: " << teid << " cellId: " << m_cellId);
	}
}
...
EpcEnbApplication::SendToS1uSocket (Ptr<Packet> packet, uint32_t teid)
{
	...
	uint32_t flags = 0;
	m_s1uSocket->SendTo (packet, flags, InetSocketAddress (m_sgwS1uAddress, m_gtpuUdpPort)); //send to GTP-U layer of eNB through UDP port
}
...
void
EpcEnbApplication::EnbS1apRcvPMReportNotify(uint16_t cellId, SONCommon::SONMsgInfo msgInfo, SONCommon::PerformanceReport pmReport){
	m_s1apSapMme->EnbS1apSendPMReportNotifyMme(cellId, msgInfo, pmReport);
}

void
EpcEnbApplication::EnbS1apRecvDSONParamResp(uint16_t cellId, SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList){
	m_s1apSapMme->EnbS1ApSendDSONParamRespMme(cellId,msgInfo,paramList);
}

void
EpcEnbApplication::EnbS1apRecvUpdateParamConfig(SONCommon::SONMsgInfo msgInfo, SONCommon::SONParamList paramList){
	m_s1SapUser->EnbS1apSendUpdateParamConfigEnbRrc(msgInfo,paramList);
}

void
EpcEnbApplication::EnbS1apRecvDSONParamReq(SONCommon::SONMsgInfo msgInfo){
	m_s1SapUser->EnbS1apSendDSONParamReqEnbRrc(msgInfo);
}
