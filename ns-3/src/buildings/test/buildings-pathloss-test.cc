...
//  AddTestCase (new BuildingsPathlossTestCase (freq, 9, 11, UrbanEnvironment, LargeCity, 183.90, "ITU1411 NLOS Indoor -> Outdoor"), TestCase::QUICK);
  ...
  AddTestCase (new BuildingsPathlossTestCase (2.1140e9, 12, 13, UrbanEnvironment, LargeCity, 76.576, "OH Urban Large city"), TestCase::QUICK);
}
...
//  building1->SetNFloors (3);
  ...
  building1->SetBoundaries (Box (0, 40, 0, 40, 0, 40)); //building 40*40*40
  building1->SetBuildingType (Building::Office);
  building1->SetExtWallsType (Building::ConcreteWithWindows);
  building1->SetNFloors (10);
  building1->SetNRoomsX(8);
  building1->SetNRoomsY(8);
  ...
  /**************THANG**************/
    std::ofstream outFile;
        outFile.open ("ThangBuildingsPathloss.txt", std::ios_base::out | std::ios_base::trunc);
        if (!outFile.is_open ()){
            NS_LOG_ERROR ("Can't open file " << "ThangBuildingsPathloss.txt");
      		return;
        }
        outFile << " ItuR1411 Pathloss " << loss << std::endl;
        outFile.close();
    /*********************************/
...
BuildingsPathlossTestCase::CreateMobilityModel (uint16_t index)
{
  ...
  double hm = 1;
  double hb = 30;
  double henbHeight = 10.0; //enb below roof top
  ...
  switch (index)
    ...
    case 12:
        mm = CreateObject<ConstantPositionMobilityModel> ();
        mm->SetPosition (Vector (-5, 20, 5)); //lamppost
        break;

    case 13:
        mm = CreateObject<ConstantPositionMobilityModel> ();
        mm->SetPosition (Vector (40, 50, 1)); //lamppost
        break;
    /***************THANG***************/
    default:
      mm = 0;
