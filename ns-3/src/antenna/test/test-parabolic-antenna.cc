...
ParabolicAntennaModelTestCase::DoRun ()
{
  ...
  outFile.open ("ParabolicAntennaModel.txt", std::ios_base::out | std::ios_base::trunc);
  if (!outFile.is_open ()){
	  NS_LOG_ERROR ("Can't open file " << "ParabolicAntennaModel.txt");
	  return;
  }
  outFile << " ItuR1411 actualGain " << actualGain << std::endl;
  outFile.close();
  /*********************/
...
{ 
  /*************THANG**************/
	AddTestCase (new ParabolicAntennaModelTestCase (Angles (DegreesToRadians (100),    0),       60,        -150,       20,             0,     EQUAL), TestCase::QUICK);
  /********************************/
