...
NullMessageSimulatorImpl::ProcessOneEvent (void)
{
  ...
  next.impl->Invoke ();
  next.impl->Unref ();
  NS_LOG_UNCOND("NullMessageSimulatorImpl::ProcessOneEvent next time stamp " << next.key.m_ts << " context "
		  << next.key.m_context << " event queue id " << next.key.m_uid);
}
...
NullMessageSimulatorImpl::ScheduleNullMessageEvent (Ptr<RemoteChannelBundle> bundle)
{
  ...
  NS_LOG_UNCOND("NullMessageSimulatorImpl::ScheduleNullMessageEvent time " << time.GetMilliSeconds() << " systemId " << bundle->GetSystemId() << " eventId " << bundle->GetEventId().GetUid());
}
...
NullMessageSimulatorImpl::RescheduleNullMessageEvent (Ptr<RemoteChannelBundle> bundle)
{
  ...
  bundle->SetEventId (Simulator::Schedule (time, &NullMessageSimulatorImpl::NullMessageEventHandler, 
                                           this, PeekPointer(bundle)));
  NS_LOG_UNCOND("NullMessageSimulatorImpl::RescheduleNullMessageEvent time " << time.GetMilliSeconds() << " systemId " << bundle->GetSystemId()
		  << " eventId " << bundle->GetEventId().GetUid());
}
...
NullMessageSimulatorImpl::RescheduleNullMessageEvent (uint32_t nodeSysId)
{
  ...
  NS_LOG_UNCOND("NullMessageSimulatorImpl::RescheduleNullMessageEvent  systemId " << nodeSysId << " evenId " << bundle->GetEventId().GetUid());
...
NullMessageSimulatorImpl::CalculateSafeTime ()
{
  ...
  NS_LOG_UNCOND("NullMessageSimulatorImpl::CalculateSafeTime safeTime " << m_safeTime.GetMilliSeconds());
...
Time NullMessageSimulatorImpl::CalculateGuaranteeTime (uint32_t nodeSysId)
{
  ...
  NS_LOG_UNCOND("NullMessageSimulatorImpl::CalculateGuaranteeTime safeTime ");
