...
#ifdef NS3_MPI
  NS_LOG_UNCOND("NullMessageMpiInterface::Enable ");//THANG
...
#ifdef NS3_MPI
  NS_ASSERT (g_enabled);
  NS_LOG_UNCOND("NullMessageMpiInterface::InitializeSendReceiveBuffers ");//THANG
...
#ifdef NS3_MPI
  NS_LOG_UNCOND("NullMessageMpiInterface::SendPacket Node "<< node << " dev " << dev << " timeStep " << rxTime.GetTimeStep ());
...
{
  NS_LOG_FUNCTION (guarantee_update.GetTimeStep () << bundle->GetSystemId());

  NS_ASSERT (g_enabled);

#ifdef NS3_MPI
  NS_LOG_UNCOND("NullMessageMpiInterface::SendNullMessage guarantee update time " << guarantee_update.GetTimeStep () << " systemId " << bundle->GetSystemId());
  NullMessageSentBuffer sendBuf;
  g_pendingTx.push_back (sendBuf);
  std::list<NullMessageSentBuffer>::reverse_iterator iter = g_pendingTx.rbegin (); // Points to the last element
...
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_LOG_UNCOND("NullMessageMpiInterface::ReceiveMessagesBlocking ");
...
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_LOG_UNCOND("NullMessageMpiInterface::ReceiveMessagesNonBlocking ");
          ...
          if (rxTime > Time (0))
            {
              ...
              NS_LOG_UNCOND("NullMessageMpiInterface::ReceiveMessages NodeId "<< pNode->GetId () << " rxTime " << rxTime - Simulator::Now ());
            }
